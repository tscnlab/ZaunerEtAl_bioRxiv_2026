...
P_B <-
  ...
  mutate(site = fct(site, levels = names(melidos_cities) |> sort()) |> fct_rev()) |> 
...
P_D_data <-
      light_glasses_processed2 |> 
      ungroup() |> 
      mutate(across(c(dawn, dusk),
                    LightLogR:::datetime_to_circular)) |> 
      aggregate_Date(
        unit = "15 mins",
        numeric.handler = \(x) {
          if(inherits(x, "circular")) return(na_dbl)
          median(x, na.rm = TRUE)
          },
        dusk2 = mean(dusk, na.rm = TRUE) |> LightLogR:::circular_to_hms(),
        dawn2 = mean(dawn, na.rm = TRUE) |> LightLogR:::circular_to_hms(),
...
P_D_data |> 
      ...
      geom_line(data = P_D_data_site, aes(y=MEDI, colour = site), linewidth = 0.75) +
...
P_E <-
      metrics_chest$data[[3]] |> 
      rbind(metrics_glasses$data[[3]]) |> 
      distinct(site, Id, Date, .keep_all = TRUE) |> 
...
Fig2_data |> 
      ...
      geom_line(aes(y = MEDI, col = site), linewidth = 0.25, layout = "fixed") +
      geom_line(aes(y = MEDI), linewidth = 1.25) +
      map(c(1,10,250), 
          \(x) geom_hline(aes(yintercept = x), col = "grey", linetype = "dashed")
      ) +
      scale_color_manual(values = melidos_colors) +
  ...
  mutate(name = 
         ...
         name = name |> 
           replace_when(
             ...
             metric_type == "level" ~ str_c(name, " (lx)"),
...
Fig3 <-
  Fig3_data |> 
  filter_out(name == "Darkest 10h mean (lx)") |> 
  pull(plot) |> 
