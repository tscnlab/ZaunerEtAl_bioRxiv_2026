...
Fig2_data |> Fig2_plot_individual("BAUA") |> ggsave(plot = _, filename = "figures/Fig2_individual/Fig2_BAUA.pdf", width = 8, height = 5)
```
...
0:60 |> 
  map(
   \(x) span_phot |> 
        ...
        group_by(lat) |> 
        summarize(min = min(photoperiod, na.rm = TRUE), 
...
metrics$data[[3]] |> 
  site_conv_mutate(rev = FALSE) |> 
  ggplot(aes(x = lat)) + 
  geom_ribbon(data = phot_potential, aes(ymin = min, ymax = max), alpha = 0.25) + 
  geom_jitter(aes(col = site, y = photoperiod), width = 0.5, height = 0, alpha = 0.5) + 
  annotate("text", y = 16, x = 15, label = "possible photoperiods", color = "darkgrey") +
  annotate("curve", y = 15.5, x = 15.5,
           xend = 20, yend = 14.3,
           curvature = 0.2,
  ...
  theme_cowplot() + 
  scale_color_manual(values = melidos_colors) +
  labs(color = "Site", y = "Photoperiod (hr)", x = "Latitude (°)",
       caption = "A jitter of width 0.5 was applied to points.\nPhotoperiod is defined as the timespan of daily sun elevation above -6°")

ggsave("figures/photoperiod_potential.pdf", width = 9)
```
