...
diagnostic_table_data <- function(run_id) {
  ...
  tibble::tibble(
    ...
    Reading = c(
      ...
      "Common and site bases overlap strongly; interpret complete curves and contrasts, not isolated coefficients."
    )
  )
}
...
The companion [H02 analysis preparation and provenance
page](../../audit/hypotheses/H02/H02_analysis_preparation.html) follows the
data-to-result chain, identifies every executable script and central
parameter, and displays intermediate input and diagnostic checks. It does not
rerun the fitted models or production bootstraps.
:::
...
The scientific question is whether daily melEDI patterns differ among sites,
how much people within a site differ from one another, and whether those
within-site differences are larger than the differences among sites.
...
BH-adjusted $p$ = `r fmt_p_value(site_test$p_adjusted, site_test$p_adjusted < 0.05)`).
...
Near-eye measurements are primary because they are closer to the light
reaching the eye. Chest measurements are analysed separately as complementary
evidence; the two placements are never pooled. During reported sleep, both
placements characterize the bedside light environment rather than light at
...
#| tbl-cap: "H02 estimand and model deviations from preregistration."
...
The first term is the **global time effect**: the daily pattern shared across
sites, which joins smoothly at midnight. The `sz` term estimates how each site
departs from that global pattern. Because those departures sum to zero, the
global time effect is centred equally across sites rather than being the curve
of a reference site or an observation-weighted site average. The participant term gives
each person a smooth departure from the site pattern. Participant identifiers
are site-prefixed, so people remain nested within sites. The final term allows
each participant-day to shift up or down without changing its shape.
...
The model was fitted with `mgcv::bam()` under fREML and a boundary-aware
AR(1) working correlation. Site curves and their pointwise conditional 95%
intervals were obtained from the fitted linear-predictor matrix and full
coefficient covariance. Smoothing parameters are treated as fixed in those
pointwise intervals. `gratia` was used for residual appraisal and
...
and participant-curve variation is the average—giving every site equal
weight—of the within-site variance among participant curves, averaged over the
same clock grid. Participant-day variation likewise gives each site equal
weight when averaging the variance of day-specific intercept contributions.
Their ratios compare fitted curve
dispersion on the modelling scale. They are not proportions of response
variance and are not “variance explained.”
...
orders of entry. This allocation is descriptive: it is neither causal
importance nor cross-validated predictive importance.
...
variation_table_data(near_id) |>
  ...
  gt::tab_source_note(
    source_note = paste(
      ...
      "units; ratios are unitless. CIs use 2,000 hierarchical resamples."
    )
  ) |>
  h02_gt()
```
...
Panel A shows the global time effect. Panel B overlays the fitted
participant curves. Panel C shows the absolute fitted curve for each site;
each facet states its fitted participant-day count. Panel D shows the site's
fitted curve divided by the global time effect. A factor of 2 means twice the
fitted $\mathrm{melEDI}+0.1\ \mathrm{lx}$ quantity; a factor of 0.5 means
half. Grey vertical bands show the mean civil-night intervals.

Ribbons in A, C, and D are pointwise conditional 95% intervals. Red segments
in D identify individual 30-minute bins whose interval excludes one. These
segments are local descriptions, not a jointly multiplicity-controlled test
across the day or across sites.

```{r}
#| label: fig-h02-near-patterns
#| fig-cap: "Daily near-eye melEDI patterns and site departures."
#| fig-alt: "Four-panel near-eye daily-pattern figure. Panel A shows the global time effect for 816 participant-days. Panel B shows participant curves. Panel C shows nine site curves with a participant-day count inside each site facet. Panel D shows site-to-global-time-effect factors with pointwise intervals."
#| out-width: "88%"
#| fig-align: center
...
#| fig-cap: "Near-eye model diagnostics."
#| fig-alt: "Five-panel diagnostic figure for the near-eye GAMM. Panels A to D show a normal QQ plot, residuals versus linear predictor, residual histogram, and observed versus fitted values using Pearson residuals. Panel E shows residual autocorrelation before and after the boundary-aware AR1 correction."
#| out-width: "88%"
#| fig-align: center
...
#| tbl-cap: "Near-eye fit and diagnostic summary."
...
The residual diagnostics are not ideal. The QQ plot has an S-shaped departure
...
Some participant-days retain positive lag-1 correlation. In addition, formal
concurvity is high between the common and site time bases even though the
...
observations, and `r chest_counts$sites` sites. Tübingen is absent because
...
#| fig-cap: "Daily chest melEDI patterns and site departures."
#| fig-alt: "Four-panel chest daily-pattern figure. Panel A shows the global time effect for 902 participant-days. Panel B shows participant curves. Panel C shows eight site curves with a participant-day count inside each site facet. Panel D shows site-to-global-time-effect factors with pointwise intervals."
#| out-width: "88%"
#| fig-align: center
...
#| fig-cap: "Chest model diagnostics."
#| fig-alt: "Five-panel diagnostic figure for the chest GAMM. Panels A to D show a normal QQ plot, residuals versus linear predictor, residual histogram, and observed versus fitted values using Pearson residuals. Panel E shows residual autocorrelation before and after the boundary-aware AR1 correction."
#| out-width: "88%"
#| fig-align: center
...
#| tbl-cap: "Chest fit and diagnostic summary."
...
non-normal tails, fitted-dependent residual spread, and high common/site
concurvity remain. The chest model is therefore corroborating evidence with
the same interpretive limitations as the near-eye model.

## Direct comparison of near-eye and chest patterns

The placement-matched analysis retained exactly the same 112 participants,
643 participant-days, and 29,786 30-minute observations at eight sites for
near-eye and chest fits. Their observation keys match one-to-one. The two
placements were fitted separately with the same response definition,
...
#| fig-cap: "Matched near-eye and chest site curves with pointwise 95% intervals (112 participants; 643 participant-days; 29,786 paired observations)."
#| fig-alt: "Eight site facets compare separately fitted near-eye and chest daily melEDI curves for the same 112 participants, 643 participant-days, and 29,786 paired half-hour observations. Near-eye curves are solid blue and chest curves are dashed orange; translucent ribbons show component pointwise conditional 95% intervals."
#| fig-width: 11
#| fig-height: 7
...
The ribbons are component pointwise conditional 95% intervals for each
...
**gap-timing-unaware dataset** is the predefined data-preparation sensitivity.
...
elsewhere in this report. The cyclic-basis analysis forces site and
participant deviations to meet at midnight. The placement-matched analyses
restrict near-eye and chest fits to the same participant-day-clock
observations.
...
The daily profile is dominated by the global time effect, but the
remaining heterogeneity is not mainly geographic. People within the same site
...
interval. A reader should not interpret every red segment as a separate
familywise-controlled discovery. The single confirmatory multiplicity family
is the near-eye omnibus site-pattern test (`H02-F1-site-pattern`); it contains
one test, so BH adjustment leaves its p-value unchanged.
...
nine near-eye sites, and the conditional—not model-refitting—nature of the
bootstrap intervals.
