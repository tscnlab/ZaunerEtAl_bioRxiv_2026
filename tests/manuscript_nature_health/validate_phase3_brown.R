...
if (length(args) != 1L) {
  stop("Usage: Rscript --vanilla tests/manuscript_nature_health/validate_phase3_brown.R <repository-root>")
}
...
required_files <- c(
  ...
  file.path(manuscript_dir, "ZaunerEtAl2026_NatHealth_phase3_brown.qmd"),
  ...
  file.path(manuscript_dir, "supplementary_information_standalone.qmd"),
  ...
  file.path(manuscript_dir, "_output", "ZaunerEtAl2026_NatHealth_phase3_brown.html"),
  file.path(manuscript_dir, "_output", "supplementary_information_standalone.html"),
  file.path(audit_dir, "phase3_brown_paragraph_claim_audit.csv"),
  file.path(audit_dir, "phase3_brown_protected_number_audit.csv"),
  file.path(audit_dir, "brown_stage3_acceptance_integration_2026-08-21.md"),
  file.path(audit_dir, "phase3_brown_clarity_changes.md"),
...
qmd_path <- file.path(manuscript_dir, "ZaunerEtAl2026_NatHealth_phase3_brown.qmd")
phase3_qmd_path <- file.path(manuscript_dir, "ZaunerEtAl2026_NatHealth_phase3.qmd")
phase2_qmd_path <- file.path(manuscript_dir, "ZaunerEtAl2026_NatHealth.qmd")
html_path <- file.path(manuscript_dir, "_output", "ZaunerEtAl2026_NatHealth_phase3_brown.html")
phase3_html_path <- file.path(manuscript_dir, "_output", "ZaunerEtAl2026_NatHealth_phase3.html")
...
assert_true(grepl("    - ZaunerEtAl2026_NatHealth_phase3_brown.qmd", config, fixed = TRUE),
            "The manuscript configuration does not select the Brown-integrated Phase 3 source.")
assert_true(!grepl("    - ZaunerEtAl2026_NatHealth_phase3.qmd", config, fixed = TRUE),
            "The preserved pre-Brown Phase 3 source remains in the active render list.")
...
assert_true(!grepl("```[[:space:]]*\\{", qmd),
            "The Brown-integrated narrative source unexpectedly contains an executable code block.")

prohibited_patterns <- c(
  "\u2014",
  paste0("des", "tiny"),
  "(?<![A-Za-z0-9_/-])H(0[1-9]|1[01])(?![A-Za-z0-9_/-])",
...
prohibited_labels <- c(
  ...
  "internal workflow identifier", "internal Brown-analysis identifier", "incorrect recruitment phrase", "incorrect eligibility framing",
...
assert_true(length(paragraph_ids) == 76L, "The manuscript does not contain the expected 76 audited paragraphs.")
assert_true(!anyDuplicated(paragraph_ids), "Paragraph audit identifiers are duplicated in the manuscript.")

paragraph_audit <- read.csv(file.path(audit_dir, "phase3_brown_paragraph_claim_audit.csv"),
                            stringsAsFactors = FALSE, check.names = FALSE)
number_audit <- read.csv(file.path(audit_dir, "phase3_brown_protected_number_audit.csv"),
                         stringsAsFactors = FALSE, check.names = FALSE)
...
assert_true(nrow(paragraph_audit) == 76L, "The paragraph claim audit does not contain 76 rows.")
...
citation_text <- gsub("@(fig|tbl)-[A-Za-z0-9_-]+", "", citation_text, perl = TRUE)
...
# The retained CIE standard uses a manuscript-local key solely to preserve the
# standard identifier and official title capitalisation under the Nature CSL.
retained_old_keys[retained_old_keys == "cies02"] <- "cie2018s026"
assert_true(length(citation_keys) == 90L && all(retained_old_keys %in% citation_keys),
            "The Brown-integrated bibliography does not retain the expected 83 old sources among 90 citations.")
...
required_additional_keys <- c(
  "cie2018s026", "zauner2026placementpreprint", "huss2019corrected", "wallace2025",
...
assert_true(grepl(
  ...
  additional_bib,
  fixed = TRUE
), "The manuscript-local CIE standard title is missing or misstyled.")

required_strings <- c(
  "184 contributed 1,478 participant-days",
  "141 adults and 816 participant-days",
  "complementary chest dataset 154 and 902",
  "participant curves plus day shifts received 9.49 times the site credit",
  "24.0% of valid waking minutes reached the recommended daytime range",
  "Near-eye in-sample R² was 0.775",
  "78.9% for the shared local-clock curve",
  ...
  "intercept added 0.080 on the model-based variance scale",
  "Lag-one residual correlation was 0.288",
  ...
  "protocol was first trialled with participants at the Tübingen (DE) site",
  ...
  "accepted Brown recommendation-adherence results and the author-approved main displays are integrated in this source",
  "main adherence model included 2,216 state periods",
  "state-by-site-by-day-type interaction met the FDR criterion",
  "separate 27-comparison FDR family",
  "descriptive response-scale partition based on point estimates",
  "We withheld the within-participant day-level claim",
  "each 10 percentage points higher observed average Daytime adherence",
  "actual-date dependence",
  "Chest adherence retained the main Daytime and Pre-sleep directions",
  "no sleep ocular claim",
  "no health outcome"
)
...
approved_display_ids <- c(
  "fig-study-overview", "tbl-participant-site", "tbl-brown-adherence",
  "fig-daily-architecture", "tbl-metric-context", "fig-activity-context"
)
for (display_id in approved_display_ids) {
  assert_true(count_matches(paste0("\\{#", display_id, "(?:[[:space:]}])"), qmd) == 1L,
              paste("An approved main display is missing or duplicated:", display_id))
  assert_true(count_matches(paste0("@", display_id, "\\b"), qmd) == 1L,
              paste("An approved main display is not referenced exactly once:", display_id))
}
...
html_inputs <- c(
  qmd_path, config_path, additional_bib_path, root_bib_path,
  file.path(manuscript_dir, "manuscript_displays.css"),
  file.path(manuscript_dir, "supplementary_information_outline.qmd"),
  list.files(file.path(manuscript_dir, "display_assets"), full.names = TRUE)
)
...
assert_true(file.info(phase3_qmd_path)$size > 0 && file.info(phase3_html_path)$size > 0,
            "The preserved pre-Brown Phase 3 source or render is missing.")
...
cat("Brown-integrated Phase 3 manuscript validation: PASS\n")
