...
# REPORT-017 order 37 source-only verification for the main hourly H06 pair.
...
test_rel <- "tests/hypotheses/H06/test_h06_report017_source_harmonization.R"
evidence_rel <- "audit/hypotheses/H06/report017_order37"
...
figure_inventory_path <- file.path(evidence_dir, "stored_figure_inventory.csv")
package_versions_path <- file.path(evidence_dir, "package_versions.csv")
allow_list_path <- file.path(evidence_dir, "editorial_allow_list.csv")
...
record_check(
  identical(chunk_labels(current_result_chunks), expected_result_chunks),
  "RESULT_CHUNK_ORDER",
...
record_check(
  identical(chunk_labels(current_companion_chunks), expected_companion_chunks),
...
record_check(
  identical(result_labels[startsWith(result_labels, "tbl-")], result_table_expected),
  "RESULT_TABLE_ENDPOINTS",
  "The 11 result table endpoints or their order differ"
)
record_check(
  identical(result_labels[startsWith(result_labels, "fig-")], result_figure_expected),
...
# Reverse the exact patch in a temporary directory and compare the sealed
# pre-edit and final structures. No project file is modified.
reverse_root <- tempfile("h06-order37-reverse-")
...
record_check(
  identical(patch_status, 0L),
  "REVERSE_PATCH",
  paste(c("Reverse patch failed", patch_output), collapse = " | ")
)
...
for (index in seq_len(nrow(expected_pre))) {
  ...
  record_check(
    ...
    paste0("REVERSE_IDENTITY_", index),
    paste("Reverse substitution did not reproduce", expected_pre$path[[index]])
  )
}
...
if (identical(patch_status, 0L)) {
  ...
  preservation_pairs <- list(
    ...
    companion_prepared_refs = list(prepared_refs(original_companion_text), prepared_refs(current_companion_text)),
...
for (pattern in forbidden_reader_patterns) {
  record_check(
    !grepl(pattern, reader_pair, ignore.case = FALSE, perl = TRUE),
...
for (token in required_result_language) {
  record_check(
    grepl(token, result_reader, fixed = TRUE),
...
for (token in required_companion_language) {
  record_check(
    grepl(token, companion_reader, fixed = TRUE),
...
record_check(
  nrow(protected) == 80L && !anyDuplicated(protected$path),
  "PROTECTED_INVENTORY_SHAPE",
  paste("Expected 80 unique protected rows, observed", nrow(protected))
)
...
execution_lines <- c(
  "# H06 order-37 one-shot execution record",
  "",
  ...
  "",
  "```sh",
  "Rscript tests/hypotheses/H06/test_h06_report017_source_harmonization.R",
  "```",
  "",
  ...
  "The exact source patch was reversed only inside an R temporary directory.",
...
source_manifest_rel <- file.path(evidence_rel, "order37_source_manifest.csv")
...
source_roles <- c(
  ...
  setNames(
    c("one-shot combined defect list", "one-shot execution record"),
    c(defect_rel, execution_rel)
  )
)
...
if (nrow(failures)) {
  message("H06 REPORT-017 order-37 source-only verification: FAIL")
...
message(
  "H06 REPORT-017 order-37 source-only verification: PASS (",
  checks_run,
