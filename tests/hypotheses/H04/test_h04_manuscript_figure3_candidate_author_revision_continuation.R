...
revision_evidence_directory <- file.path(
  authorized_evidence_directory,
  "author_revision_verifier_continuation_2026_08_31"
)
if (
  !identical(
    normalizePath(
    ...
    !identical(
      ...
      basename(revision_evidence_directory)
    )
) {
  stop(
...
builder_path <- input_path(
  paste0(
    ...
    "build_h04_manuscript_figure3_candidate_author_revision.R"
  )
)
test_path <- input_path(
  paste0(
    "tests/hypotheses/H04/",
    "test_h04_manuscript_figure3_candidate_author_revision.R"
  )
)
...
record_check(
  ...
  identical(input_identity_output$path, frozen_inputs$path) &&
    ...
    identical(
      ...
      unname(frozen_inputs$sha256_observed)
    ),
...
svg_cells <- do.call(
  ...
  lapply(seq_along(cell_groups), function(index) {
    ...
    wrapped_site_average <- identical(row_type, "site_average") &&
      !identical(activity, "At home")
    tibble::tibble(
      row_type = row_type,
      site = xml2::xml_attr(group, "data-site"),
      activity = activity,
      display_value = text_tokens[[1]],
      confidence_interval = if (identical(row_type, "site_average")) {
        if (wrapped_site_average) {
          paste(text_tokens[[2]], text_tokens[[3]])
        } else {
          text_tokens[[2]]
        }
      } else {
      ...
      fdr_p = if (!estimable) {
        NA_character_
      } else if (identical(row_type, "site_average")) {
        if (wrapped_site_average) text_tokens[[4]] else text_tokens[[3]]
      } else {
        text_tokens[[2]]
      },
      estimable = estimable,
...
expected_display_value <- ifelse(
  candidate_panel_d$row_type == "site_deviation" &
    candidate_panel_d$estimable,
  paste0(
    candidate_panel_d$estimate,
    " (",
    sub("^95% CI\\s*", "", candidate_panel_d$confidence_interval),
    ")"
  ),
  candidate_panel_d$estimate
)
expected_vector_interval <- ifelse(
  candidate_panel_d$row_type == "site_average",
...
record_check(
  ...
  nrow(svg_cells) == nrow(candidate_panel_d) &&
    ...
    identical(svg_cells$display_value, expected_display_value) &&
    identical(
      svg_cells$confidence_interval,
      expected_vector_interval
    ) &&
...
record_check(
  ...
  paste(
    ...
    "authorized uppercase-tag moves from right to left and wrapper removal."
  )
)
...
record_check(
  ...
  paste(
    "Decoded pixels in panels A-C are identical outside the six old and new",
    "tag boxes, and every authorized box contains a tag-placement change."
  )
)
...
upper_tag_nodes <- xml2::xml_find_all(
  composite_svg,
  ".//*[local-name()='text' and (@text-anchor='start') and (text()='A' or text()='B' or text()='C' or text()='D')]"
)
upper_tags <- xml2::xml_text(upper_tag_nodes)
upper_tag_x <- xml2::xml_attr(upper_tag_nodes, "x")
record_check(
  "uppercase_left_panel_tags",
  identical(upper_tags, c("A", "B", "C", "D")) &&
    identical(upper_tag_x, rep("16.00", 4L)),
  "The vector candidate contains one uppercase left-side tag for each panel A-D."
)

support_text_nodes <- xml2::xml_find_all(
  panel_d_svg,
  '//*[contains(concat(" ", normalize-space(@class), " "), " category-support ")]/*[local-name()="text"]'
)
support_y <- as.numeric(xml2::xml_attr(support_text_nodes, "y"))
table_background <- xml2::xml_find_first(
  panel_d_svg,
  '//*[local-name()="rect" and @y="60" and @fill="#D0D4D8"]'
)
if (inherits(table_background, "xml_missing")) {
  table_background <- xml2::xml_find_first(
    panel_d_svg,
    '//*[local-name()="rect" and @y="60"]'
  )
}
record_check(
  "panel_d_author_geometry",
  length(support_y) == 15L &&
    identical(sort(unique(support_y)), c(95, 110, 125)) &&
    identical(xml2::xml_attr(table_background, "x"), "68.89"),
  paste(
    "Panel D begins at the plot-panel left edge and places the unchanged",
    "three support lines directly beneath each unchanged category heading."
  )
)
...
record_check(
  ...
  stringr::str_detect(reader_text, stringr::fixed("Site-average estimates")) &&
    stringr::str_detect(
      reader_text,
    ...
    all(vapply(
      c("**A**", "**B**", "**C**", "**D**"),
      function(tag) stringr::str_detect(caption, stringr::fixed(tag)),
      logical(1)
    )) &&
    stringr::str_detect(
      caption,
      stringr::fixed("95% confidence intervals")
    ),
  paste(
