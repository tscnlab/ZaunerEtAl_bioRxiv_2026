...
expected_png_dimensions <- function(family) {
  if (identical(family, "primary")) {
    return(c(width = 2141L, height = 1486L))
  }
  if (identical(family, "site")) {
    return(c(width = 2141L, height = 1700L))
  }
  c(width = 2141L, height = 2582L)
}
...
verify_temporary_inventory <- function(temporary_dir) {
  ...
  assert_true(
    identical(
      unname(vapply(inventory$path, sha256_file, character(1))),
      inventory$sha256
    ) &&
      identical(
        unname(vapply(inventory$path, file_bytes, numeric(1))),
        inventory$bytes
      ),
