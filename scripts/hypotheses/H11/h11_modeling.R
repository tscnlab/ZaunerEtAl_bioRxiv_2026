...
h11_paths <- function(root) {
  list(
    model_data = file.path(root, "results/intermediate/model_data/H11/fitted"),
    models = file.path(root, "results/models/H11/fitted"),
    diagnostics = file.path(root, "results/csv/diagnostics/H11/fitted"),
    tables = file.path(root, "results/tables/H11/fitted"),
    figures = file.path(root, "results/images/H11/fitted"),
    source_data = file.path(root, "results/csv/source_data/H11/fitted")
  )
}

h11_create_directories <- function(paths) {
  invisible(lapply(paths, dir.create, recursive = TRUE, showWarnings = FALSE))
}

h11_registry <- function(root) {
  tibble::tribble(
    ...
    "alternative_preprocessing", "glasses", "all_available",
    "alternative_preprocessing_near_eye",
    "H11-S-MP-glasses-global", "H11-S-MP-glasses-level-shape",
    "alternative_preprocessing__chest__all_available",
    "alternative_preprocessing", "chest", "all_available",
    "alternative_preprocessing_chest",
...
h11_prepare_frame <- function(frame, demographics, run_id) {
  required_frame <- c(
  ...
  if (anyDuplicated(demo[c("site", "Id")])) {
    h11_abort("Demographics is not many-to-one by site and participant")
  }
  ...
  if (!identical(output$.h11_original_row, seq_len(nrow(output)))) {
    h11_abort("H02 row order changed while preparing %s", run_id)
  }
  output <- output |>
    dplyr::mutate(
      sex = factor(
  ...
  if (
    ...
    h11_abort("H11 factor or response contract failed for %s", run_id)
  }
  output
}

h11_sample_row <- function(data, run) {
...
h11_formula_equal <- function(x, y) {
  identical(
...
h11_validate_fit <- function(
    ...
    model_id) {
  if (!inherits(fit, "bam")) {
    h11_abort("Model %s is not an mgcv BAM fit", model_id)
  }
  if (!h11_formula_equal(fit, formula)) {
    h11_abort("Model %s has the wrong formula", model_id)
  }
  if (
    ...
    h11_abort("Model %s is not aligned to its exact frame", model_id)
  }
  observed_method <- as.character(fit$method)
  if (!identical(observed_method, method)) {
    h11_abort(
      "Model %s uses %s rather than %s",
      model_id,
      observed_method,
      method
    )
  ...
  if (
    ...
    h11_abort("Model %s has the wrong AR(1) rho", model_id)
  }
  invisible(TRUE)
}

h11_fit_and_save <- function(
    formula,
    data,
    method,
    rho,
...
h11_model_row <- function(result, run, formula_id) {
  fit <- result$fit
...
h11_adjust_multiplicity <- function(comparisons) {
  ...
  if (any(adjusted$observed_family_n != adjusted$planned_n)) {
    h11_abort("An H11 multiplicity family has the wrong observed size")
  }
  adjusted |>
    dplyr::mutate(
      AIC_supported = NA,
...
h11_fit_run <- function(frame, demographics, run, formulas, paths) {
  data <- h11_prepare_frame(frame, demographics, run$run_id)
  frame_path <- file.path(paths$model_data, paste0(run$run_id, "__frame.rds"))
  saveRDS(data, frame_path, compress = "xz")
  sample <- h11_sample_row(data, run)

  preliminary <- h11_fit_and_save(
  ...
  if (!is.finite(rho) || abs(rho) > 0.95) {
    h11_abort("Invalid H11 rho for %s", run$run_id)
  }

  final <- h11_fit_and_save(
    formula = formulas$proposed_mpattern,
    data = data,
    method = "fREML",
    rho = rho,
  ...
  model_rows <- dplyr::bind_rows(
    h11_model_row(preliminary, run, "proposed_mpattern"),
    h11_model_row(final, run, "proposed_mpattern")
  )
  ...
  saveRDS(
    bundle,
    file.path(paths$models, run$run_id, "fit_bundle.rds"),
    compress = "xz"
  )
  bundle
}

h11_equal_site_lpmatrix <- function(fit, data, time_values) {
...
h11_robust_context <- function(fit, data, run_id) {
  started <- proc.time()[["elapsed"]]
...
h11_robust_target_covariance <- function(context, L) {
...
h11_robust_wald <- function(
    fit,
    L,
    reference_rank,
    context) {
  target <- h11_robust_target_covariance(context, L)
...
h11_robust_tests <- function(fit, data, run, context) {
  time_values <- (seq.int(0L, 1410L, by = 30L) + 15) / 60
  curve_contract <- h11_equal_site_lpmatrix(
    fit,
    data,
    time_values
  )
  ...
  if (length(male_rows) != 48L || length(female_rows) != 48L) {
    h11_abort("The robust sex-curve grid is incomplete for %s", run$run_id)
  }
  ...
  if (length(sex_index) != 1L || length(sex_smooth_number) != 1L) {
    h11_abort("Could not identify the H11 sex block for %s", run$run_id)
  }
  ...
  if (max(abs(D[, other_index, drop = FALSE])) > 1e-10) {
    h11_abort("The H11 curve contrast contains nuisance terms")
  }
  ...
  fitted_tests <- lapply(test_specs, function(spec) {
    h11_robust_wald(fit, spec$L, spec$rank, context)
  })
    ...
    function(test_id) {
      ...
      tibble::tibble(
        ...
        inferential_role = dplyr::case_when(
          ...
          TRUE ~ "diagnostic-only data-preparation sensitivity"
        )
      )
    }
  ))
  ...
  coefficient_test <- h11_robust_wald(
    fit,
    L_coefficient,
    global_rank,
    context
...
h11_pointwise_curves <- function(fit, data, run_id, robust_context) {
  time_values <- (seq.int(0L, 1410L, by = 30L) + 15) / 60
  contract <- h11_equal_site_lpmatrix(fit, data, time_values)
  L <- contract$L
  ...
  if (
    ...
    h11_abort("The sex-contrast grid is incomplete for %s", run_id)
  }
...
h11_curve_closure <- function(fit, data, run_id) {
  contract <- h11_equal_site_lpmatrix(fit, data, c(0, 24))
...
h11_pointwise_segments <- function(contrast) {
...
h11_parametric_sex <- function(fit, run_id, robust_context) {
  coefficient_names <- names(stats::coef(fit))
  index <- which(coefficient_names == "sexFemale")
  if (length(index) != 1L) {
    h11_abort("Could not identify the Female-minus-Male parametric term")
  }
  estimate <- stats::coef(fit)[index]
  L <- matrix(0, nrow = 1L, ncol = length(stats::coef(fit)))
  L[1L, index] <- 1
  covariance <- h11_robust_target_covariance(
    robust_context,
    L
  )$covariance
...
h11_curve_variation <- function(
  ...
  variance <- h11_robust_target_covariance(
    robust_context,
    gradient_L
  )$covariance[1L, 1L]
  ...
  tibble::tibble(
    ...
    confidence_interval_method = paste(
      "First-order delta-method pointwise 95% interval from the retained",
...
h11_r2_point <- function(response, baseline_prediction, full_prediction) {
  if (
    ...
    h11_abort("Effect-size predictions are invalid")
  }
  total_sum_squares <- sum((response - mean(response))^2)
  if (!is.finite(total_sum_squares) || total_sum_squares <= 0) {
    h11_abort("Effect-size response total sum of squares is not positive")
  }
...
h11_bootstrap_r2 <- function(
    ...
    replicates = 50L,
    seed) {
  if (length(replicates) != 1L || !is.finite(replicates) ||
      replicates < 1L || replicates != as.integer(replicates)) {
    h11_abort("Bootstrap replicates must be one positive integer")
  }
...
h11_effect_bootstrap_summary <- function(resampling, run_id) {
  intervals <- t(vapply(
    seq_len(ncol(resampling$draws)),
    function(index) {
      stats::quantile(
        resampling$draws[, index],
        probs = c(0.025, 0.975),
        na.rm = TRUE,
        names = FALSE
      )
  ...
  effect_size_id <- names(resampling$point)
  tibble::tibble(
    run_id = run_id,
    effect_size_id = effect_size_id,
    estimate = unname(resampling$point),
    bootstrap_lower_95 = intervals[, 1L],
    bootstrap_upper_95 = intervals[, 2L],
    unit = ifelse(
      grepl("share|partial", effect_size_id),
      "proportion",
      "R-squared"
    ),
    bootstrap_replicates = resampling$replicates,
    bootstrap_seed = resampling$seed,
    bootstrap_failed_replicates = resampling$failed_replicates,
    bootstrap_elapsed_seconds = resampling$elapsed_seconds,
    interval_method = paste(
      "Descriptive precision check, not formal inference; 95% percentile",
...
h11_residual_acf <- function(fit, data, run_id, stage, max_lag = 12L) {
  residual <- if (
...
h11_residual_summary <- function(fit, data, run_id) {
...
h11_smooth_summary <- function(fit, run_id) {
...
h11_concurvity <- function(fit, run_id) {
  started <- proc.time()[["elapsed"]]
...
h11_k_check <- function(fit, run_id, seed) {
  set.seed(seed)
...
h11_site_constraint <- function(fit, data, run_id) {
  labels <- h02_smooth_labels(fit)
  index <- which(labels == "s(time_hour,site)")
  if (length(index) != 1L) {
    h11_abort("Could not identify the inherited site-deviation smooth")
  }
  columns <- seq.int(
...
h11_diagnostic_assessment <- function(
    model_row,
    residual_acf,
    closure,
    site_constraint,
  ...
  if (nrow(final_row) != 1L) {
    h11_abort("Missing final model row for diagnostic assessment")
  }
  lag1 <- residual_acf |>
    dplyr::filter(
  ...
  tibble::tibble(
    ...
    plain_language_interpretation = if (hard_failure) {
      paste(
        ...
        "H11 inference within this model specification."
      )
    } else {
      paste(
