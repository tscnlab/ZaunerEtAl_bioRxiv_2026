...
# Finalize REPORT-011 status only after the recorded three-page A4 inspection.
...
if (!identical(as.character(getRversion()), "4.6.1")) {
  stop("H11 preparation figure QA requires R 4.6.1", call. = FALSE)
}

producer <- paste0(
  "scripts/hypotheses/H11/",
  "finalize_h11_preparation_figure_qa.R"
)
qa_path <- file.path(
  root,
...
proof_path <- file.path(
  root,
  "artifacts/12_manifests/H11/H11_preparation_figure_A4_proofs.pdf"
)
...
stopifnot(
  ...
  all(abs(qa$base_width_in - 8.5) < 1e-12),
  all(abs(qa$export_scale_multiplier - 1) < 1e-12),
  all(abs(qa$export_width_in - 8.5) < 1e-12),
  all(abs(qa$native_export_width_mm - 215.9) < 1e-8),
  all(qa$intended_display_width_mm == 170),
  all(abs(qa$display_reduction_factor - 170 / 215.9) < 1e-12),
  all(qa$smallest_central_nominal_text_pt == 12),
  all(qa$effective_final_central_text_pt >= 9),
  all(qa$smallest_essential_nominal_text_pt == 10),
  all(qa$effective_final_essential_text_pt >= 7),
  all(qa$smallest_minor_nominal_text_pt == 9),
...
qa_final <- qa |>
  dplyr::mutate(
    ...
    inspection_basis = paste(
      "Three-page A4 portrait raster proof of each exported asset reduced",
      "from 215.9 mm to 170 mm, with 20-mm side margins; inspected one",
    ...
    qa_record = paste0(
      ...
      "H11_preparation_figure_readability_qa.md"
    )
  )

invisible(write_csv_artifact(qa_final, qa_path, producer = producer))
message("H11 preparation REPORT-011 QA finalized: 3/3 figures PASS")
