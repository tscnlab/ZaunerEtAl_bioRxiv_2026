...
# in the H04 preparation companion. The proof uses only existing raster
...
if (!identical(as.character(getRversion()), "4.6.1")) {
  stop(
    sprintf("H04 requires R 4.6.1; running %s", getRversion()),
    call. = FALSE
  )
}
...
producer <- paste0(
  ...
  "build_h04_preparation_figure_readability_qa.R"
)
...
qa <- tibble::tribble(
  ...
  paste0(asset_root, "fig-h04-prep-category-support-1.png"),
  10, 6.4,
  "Six category labels, both support measures, placement legend, ticks, and paired bars must remain readable and balanced.",
  "fig-h04-prep-site-category-support",
  paste0(asset_root, "fig-h04-prep-site-category-support-1.png"),
  11, 8.2,
  "Submitted site labels, six categories, sparse circles, and the weighted-hour legend must remain distinguishable.",
  "fig-h04-prep-clock-category-support",
  paste0(asset_root, "fig-h04-prep-clock-category-support-1.png"),
  11, 8,
  "Hourly cells, category labels, selected clock ticks, support markers, and the weighted-hour legend must remain distinguishable."
)
...
if (any(!file.exists(absolute_paths))) {
  stop(
    paste0(
      "Missing H04 preparation figure asset(s): ",
...
if (
  ...
  stop("Invalid H04 preparation physical-size evidence", call. = FALSE)
}
...
if (!file.exists(proof_path) || file.info(proof_path)$size <= 0L) {
  stop("H04 preparation A4 figure proof was not created", call. = FALSE)
}
...
if (!inspection_passed) {
  stop("H04 preparation visual-inspection record is incomplete", call. = FALSE)
}
...
qa <- qa |>
  mutate(
    ...
    symlog_applicability = if_else(
      .data$figure_id == "fig-h04-prep-positive-distribution",
      paste(
...
message(
  "H04 preparation figure QA passed for ",
  nrow(qa),
