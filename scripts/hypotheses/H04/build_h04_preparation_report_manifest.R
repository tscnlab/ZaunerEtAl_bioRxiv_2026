...
producer <- paste0(
  ...
  "build_h04_preparation_report_manifest.R"
)
...
website_assets_path <- file.path(
  website_root,
  "H04_analysis_preparation_files"
)
...
if (!identical(
  ...
  stop(
    "The H04 website QMD copy is not byte-identical to its authoring source",
    call. = FALSE
  )
}
...
files <- unique(c(
  ...
  website_html_path,
  website_assets,
  file.path(root, "notebooks/hypotheses/H04.qmd"),
  file.path(root, "_build/nathealth/notebooks/hypotheses/H04.html"),
  file.path(root, "config/site_display_registry.csv"),
  file.path(root, "renv.lock"),
  file.path(root, decision_relatives),
  file.path(root, "audit/handoffs/H04_shared_change_request.md"),
...
files <- files[!normalizePath(
  files,
  winslash = "/",
  mustWork = FALSE
) %in% normalizePath(output_path, winslash = "/", mustWork = FALSE)]

if (any(!file.exists(files))) {
  missing <- files[!file.exists(files)]
  stop(
    paste0(
      "Missing H04 preparation-report provenance file(s): ",
...
role <- dplyr::case_when(
  ...
  relative == "notebooks/hypotheses/H04.qmd" ~ "result_report_source",
  ...
  startsWith(relative, "artifacts/07_models/H04/") ~
    "stored_model_output",
  startsWith(relative, "artifacts/08_diagnostics/H04/") ~
    "stored_diagnostic_output",
  startsWith(relative, "artifacts/09_tables/H04/") ~
    "stored_table_output",
  startsWith(relative, "artifacts/10_figures/H04/") ~ "reader_figure",
  startsWith(relative, "artifacts/11_source_data/H04/") ~
    "reader_figure_or_table_source_data",
  startsWith(relative, "artifacts/12_manifests/H04/") ~
    "referenced_H04_manifest",
  relative == paste0(
    "audit/hypotheses/H04/",
    "H04_preparation_figure_readability_qa.md"
  ) ~ "visual_QA_record",
  startsWith(relative, "audit/decisions/") ~ "approved_reporting_rule",
  relative == "audit/handoffs/H04_shared_change_request.md" ~
    "shared_integration_request",
...
if (
  ...
  stop("Invalid H04 preparation-report manifest", call. = FALSE)
}

invisible(write_csv_artifact(manifest, output_path, producer))
message(
  "H04 preparation-report manifest completed: ",
  nrow(manifest),
  " files"
)
