...
panel_d_lines <- c(
  panel_d_lines,
  svg_text(left_margin + 8, 110, "Site", size = 15.5, weight = "bold")
)
...
for (index in seq_along(activity_order)) {
  ...
  header_y <- if (length(header_lines) == 1L) 79 else c(72, 88)
  panel_d_lines <- c(
    panel_d_lines,
    vapply(
...
for (index in seq_along(activity_order)) {
  ...
  panel_d_lines <- c(
    ...
    svg_text(x, header_bottom + 50, cell$confidence_interval, size = 13.2),
    svg_text(x, header_bottom + 70, cell$fdr_p, size = 13.2),
    "</g>"
  )
}
...
for (site_index in seq_along(site_order)) {
  ...
  for (activity_index in seq_along(activity_order)) {
    ...
    if (!cell$estimable) {
      ...
      panel_d_lines <- c(
        panel_d_lines,
        svg_text(x, row_top + 18, cell$estimate, size = 15.2, weight = weight),
        svg_text(
          x,
          row_top + 35,
...
for (index in seq_len(nrow(tag_specification))) {
  old_tag <- sprintf(
    ...
    tag_specification$x[[index]],
    tag_specification$y[[index]],
    tag_specification$upper[[index]]
  )
  new_tag <- sub(
    paste0(">", tag_specification$upper[[index]], "</text>$"),
    paste0(">", tag_specification$lower[[index]], "</text>"),
    old_tag
  )
...
tag_pixels <- tag_specification
tag_pixels$x_start <- floor(tag_pixels$box_x * pixels_per_point) + 1L
tag_pixels$x_end <- ceiling(
  (tag_pixels$box_x + tag_pixels$box_width) * pixels_per_point
)
tag_pixels$y_start <- floor(tag_pixels$box_y * pixels_per_point) + 1L
tag_pixels$y_end <- ceiling(
  (tag_pixels$box_y + tag_pixels$box_height) * pixels_per_point
)

for (index in seq_len(nrow(tag_pixels))) {
  pixel_width <- tag_pixels$x_end[[index]] - tag_pixels$x_start[[index]] + 1L
  pixel_height <- tag_pixels$y_end[[index]] - tag_pixels$y_start[[index]] + 1L
  tile_svg <- paste0(
    ...
    pixel_height,
    "px' viewBox='0 0 ",
    tag_pixels$box_width[[index]],
    " ",
    tag_pixels$box_height[[index]],
    "'>",
    "<rect width='100%' height='100%' style='stroke:none;fill:#FFFFFF;'/>",
    svg_text(
      tag_pixels$x[[index]] - tag_pixels$box_x[[index]],
      tag_pixels$y[[index]] - tag_pixels$box_y[[index]],
      tag_pixels$lower[[index]],
      size = 14,
      weight = "bold",
      anchor = "end"
    ),
  ...
  candidate_top[
    tag_pixels$y_start[[index]]:tag_pixels$y_end[[index]],
    tag_pixels$x_start[[index]]:tag_pixels$x_end[[index]],
  ] <- tile_pixels
}
...
caption <- paste(
  paste(
    ...
    "context. Panels a-c show site-average activity-associated melEDI by",
  ...
  paste(
    ...
    "categories. Other/unspecified activity is display-only in panels a-c",
    "and absent from panel d."
  )
)
