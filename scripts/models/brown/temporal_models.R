...
sys.source("scripts/models/brown/temporal_endpoint.R", lb_endpoint_ou)
lb_bb_ou <- new.env(parent = environment())
sys.source("scripts/models/brown/temporal_beta_binomial.R", lb_bb_ou)
lb_temporal_prepare <- function(raw_frame) {
  before <- data.table::copy(raw_frame)
  stopifnot(
...
lb_temporal_fit_endpoint <- function(input, base_bundle, job) {
  ...
  criteria <- c(
    convergence = fit_diagnostics$convergence == 0L,
    gradient = is.finite(fit_diagnostics$maximum_absolute_gradient) &&
      fit_diagnostics$maximum_absolute_gradient <= 0.01,
    hessian = fit_diagnostics$positive_definite_hessian,
    covariance = finite_covariance && is.finite(min_eigen) && min_eigen > 0,
    parameters = all(is.finite(optimizer$par)),
    endpoint_coefficient = is.finite(fit_diagnostics$maximum_endpoint_coefficient) &&
      fit_diagnostics$maximum_endpoint_coefficient <= 15,
    endpoint_uncertainty = is.finite(fit_diagnostics$maximum_endpoint_standard_error) &&
      fit_diagnostics$maximum_endpoint_standard_error <= 10,
    participant_sd = is.finite(fit_diagnostics$participant_standard_deviation) &&
      fit_diagnostics$participant_standard_deviation >= 1e-4,
    OU_sd = is.finite(fit_diagnostics$ou_standard_deviation) &&
      fit_diagnostics$ou_standard_deviation >= 1e-4,
    OU_decay = is.finite(fit_diagnostics$ou_rate_per_day) &&
      fit_diagnostics$ou_rate_per_day > 0 &&
      is.finite(fit_diagnostics$ou_half_life_days) &&
      fit_diagnostics$ou_half_life_days > 0
  )
  fit_diagnostics$structural_failure <- !all(criteria)
  fit_diagnostics$fit_status <- if (fit_diagnostics$structural_failure) "structural_failure" else if (
    fit_diagnostics$maximum_absolute_gradient > 0.001 || length(warnings) > 0L
  )
    "acceptable_with_cautions" else "acceptable"
  fit_diagnostics$failure_components <- paste(names(criteria)[!criteria], collapse = ";")
  list(
    model_id = job$model_id,
    sample_id = job$sample_id,
    route = "ENDPOINT-R3",
...
lb_temporal_fit_bb <- function(input, job) {
  ...
  fit_diagnostics <- lb_bb_ou$assess_fit(
    result,
    data,
    job$model_id,
    job$sample_id,
  ...
  valid_decay <- is.finite(fit_diagnostics$ou_decay_rate_per_day) &&
    fit_diagnostics$ou_decay_rate_per_day > 0 &&
    is.finite(fit_diagnostics$ou_half_life_days) &&
    fit_diagnostics$ou_half_life_days > 0
  fit_diagnostics$decay_identifiable <- valid_decay
  fit_diagnostics$structural_failure <- fit_diagnostics$structural_failure || !valid_decay
  if (fit_diagnostics$structural_failure) fit_diagnostics$fit_status <- "structural_failure"
  list(
    model_id = job$model_id,
    sample_id = job$sample_id,
    route = job$route,
