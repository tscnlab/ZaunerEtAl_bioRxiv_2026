...
Type objective_function<Type>::operator() () {
  ...
  for (int observation = 0; observation < observations; observation++) {
    ...
    if (use_zero_component == 1 && one_active(observation) == 1) {
      ...
      pi_beta(observation) = exp(log_pi_beta);
    } else if (one_active(observation) == 1) {
      Type log_denominator = logspace_add_pair(
        Type(0),
        eta_one(observation)
      );
      log_pi_one = eta_one(observation) - log_denominator;
      log_pi_beta = -log_denominator;
      pi_zero(observation) = Type(0);
      pi_one(observation) = exp(log_pi_one);
      pi_beta(observation) = exp(log_pi_beta);
    } else {
      pi_zero(observation) = Type(0);
      pi_one(observation) = Type(0);
      pi_beta(observation) = Type(1);
      log_pi_beta = Type(0);
    }

    Type log_beta_binomial = beta_binomial_logpmf(
      Type(y(observation)),
      Type(n(observation)),
      mu(observation),
      phi(observation)
    ...
    if (use_zero_component == 1 && y(observation) == 0) {
    ...
    if (
      one_active(observation) == 1 &&
      y(observation) == n(observation)
    ) {
      observation_log_likelihood = logspace_add_pair(
        observation_log_likelihood,
        log_pi_one
  ...
  SIMULATE {
    ...
    for (int observation = 0; observation < observations; observation++) {
      ...
      if (component_draw < pi_zero(observation)) {
        ...
        component_draw <
        pi_zero(observation) + pi_one(observation)
      ) {
        y_simulated(observation) = Type(n(observation));
      } else {
        Type alpha = mu(observation) * phi(observation);
        Type beta = (Type(1) - mu(observation)) * phi(observation);
        Type probability = rbeta(alpha, beta);
