...
replica_profile_y_scale <- function(
...
make_metric_replica_panel <- function(
  metric_values,
  contract_row,
  panel_index
) {
...
make_time_series_replica_figure <- function(series, states, metrics) {
  ...
  # The submitted V0 generator explicitly dropped missing MEDI values before
  # drawing the paths. Retain the complete stored source for auditability, but
  # use the same finite plotting rows so isolated missing bins do not create
  # visual breaks that were absent from the submitted figure.
  series_plot <- series |>
  ...
  p_a <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_manual(
      values = replica_participant_palette(),
      guide = "none"
    ) +
    ggplot2::scale_colour_manual(
      values = replica_participant_palette(),
