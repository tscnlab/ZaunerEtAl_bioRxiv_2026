# Reader-facing replicas of the manuscript-generating descriptive tables and
# the fixed-example time-series source bundle. All calculations use verified
# prepared inputs supplied by build_descriptives.R.

replica_site_levels <- function() {
  c("Overall", descriptive_site_order())
}

replica_site_label <- function(site) {
  ifelse(
    site == "Overall",
    "Overall",
...
format_duration_minutes <- function(minutes) {
  ifelse(
    is.finite(minutes),
    sprintf("%02d:%02d", round(minutes) %/% 60L, round(minutes) %% 60L),
    "Not available"
  )
}
...
replica_table_row <- function(
  ...
  data.frame(
    ...
    site = site,
    reader_site = replica_site_label(site),
    display = display,
...
build_participant_site_replica <- function(
  ...
  site_rows <- lapply(replica_site_levels(), function(site_name) {
    ...
    rows <- list(
      ...
      replica_table_row(
        ...
        if (site_name == "Overall") "—" else sample$country,
        "Study site"
      ),
      replica_table_row(
        "Site specifics", "City", site_name,
        if (site_name == "Overall") "—" else sample$city,
        "Study site"
      ),
      replica_table_row(
        "Site specifics", "Coordinates", site_name,
        if (site_name == "Overall") "—" else sprintf(
          "%.1f°%s, %.1f°%s",
  ...
  dplyr::bind_rows(site_rows) |>
    dplyr::mutate(
      site = factor(.data$site, levels = replica_site_levels()),
      reader_site = factor(
        .data$reader_site,
        levels = replica_site_label(replica_site_levels())
      )
    ) |>
    dplyr::arrange(
...
replica_metric_contract <- function() {
  data.frame(
    category = c(
...
build_metric_replica <- function(metric_summary) {
  ...
  metric_summary |>
    ...
    dplyr::mutate(
      site = factor(as.character(.data$site), levels = replica_site_levels()),
      reader_site = replica_site_label(as.character(.data$site)),
      median_formatted = mapply(
        format_replica_metric_value, .data$median, .data$metric_id,
        USE.NAMES = FALSE
      ),
      q1_formatted = mapply(
        format_replica_metric_value, .data$q1, .data$metric_id,
        USE.NAMES = FALSE
      ),
      q3_formatted = mapply(
        format_replica_metric_value, .data$q3, .data$metric_id,
        USE.NAMES = FALSE
      ),
      mean_formatted = mapply(
        format_replica_metric_value, .data$mean, .data$metric_id,
        USE.NAMES = FALSE
      ),
      sd_formatted = mapply(
        format_replica_metric_spread, .data$sd, .data$metric_id,
        USE.NAMES = FALSE
      ),
      display = paste0(
...
build_recommendation_replica <- function(recommendation) {
  main <- recommendation |>
    dplyr::mutate(site = as.character(.data$site)) |>
    dplyr::arrange(
      factor(.data$site, levels = replica_site_levels()),
...
replica_time_series_ids <- function() {
  c(
...
read_gap_timing_unaware_near_eye <- function(root) {
  ...
  if (!identical(observed_sha256, contract$expected_sha256[[1L]])) {
    stop(
      "Gap-timing-unaware near-eye source hash changed: ", observed_sha256,
      call. = FALSE
    )
  }
...
build_time_series_replica_sources <- function(root) {
  ids <- replica_time_series_ids()
  ...
  selected_dates <- source$data |>
    ...
    dplyr::mutate(
      ...
      selection_rule = paste0(
        "Same seven submitted exemplars and study days 2–6; displayed ",
  ...
  if (nrow(selected_dates) != expected_rows) {
    stop("Time-series replica selection does not contain 7 × 5 days", call. = FALSE)
  }
