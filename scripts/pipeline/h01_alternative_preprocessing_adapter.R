...
h01_alternative_preprocessing_scenario_id <- function() {
  alternative_preprocessing_scenario_id()
}

h01_alternative_preprocessing_model_implementation_id <- function() {
  alternative_preprocessing_model_implementation_id()
}

h01_alternative_preprocessing_support_reason <- function() {
  paste0(
    "alternative_preprocessing_artifacts_do_not_retain_",
    "exact_support_minutes"
  )
}

h01_alternative_preprocessing_site_date_path <- function(root) {
  stats::setNames(
    file.path(
      root,
      "results", "intermediate/model_data",
      "scenarios",
      h01_alternative_preprocessing_scenario_id(),
      "participant_day_metrics.rds"
    ),
    "alternative_preprocessing_participant_day"
  )
}

h01_alternative_preprocessing_missing_metric_reason <- function() {
  "alternative_preprocessing_nonestimable_reason_not_recorded"
}

h01_alternative_preprocessing_measurement_construct <- function(position) {
  dplyr::case_when(
    position == "glasses" ~
...
h01_alternative_preprocessing_timing_metrics <- function() {
  c(
    "m10_midpoint",
    "l10_midpoint",
...
h01_alternative_preprocessing_output_paths <- function(root = project_root(), output_root = root) {
  root <- normalizePath(root, winslash = "/", mustWork = TRUE)
  output_root <- normalizePath(output_root, winslash = "/", mustWork = TRUE)
  source_paths <- alternative_preprocessing_output_paths(root, root)
  scenario_root <- file.path(output_root, "results", "intermediate", "model_data", "H01",
                             "scenarios", h01_alternative_preprocessing_scenario_id())
  tables <- c("model_rows", "metric_contract", "sample_flow", "exclusion_reasons",
              "scenario_status", "predictor_centers", "variable_dictionary")
  list(root = root, output_root = output_root, source = source_paths,
       scenario_root = scenario_root, support_root = scenario_root,
       rds = file.path(scenario_root, "H01.rds"),
       csv = stats::setNames(file.path(scenario_root, paste0(tables, ".csv")), tables))
}

h01_alternative_preprocessing_selected_crosswalk <- function(
  crosswalk,
  main_contract
) {
  assert_columns(
    ...
    object = "alternative-preprocessing metric crosswalk"
  )
  selected <- crosswalk |>
  ...
  if (
    ...
    h01_abort(
      "Alternative-preprocessing H01 metrics differ from the 17-metric contract"
    )
  }
  timing <- selected$metric_id %in%
    h01_alternative_preprocessing_timing_metrics()
  ...
  if (
    ...
    h01_abort(
      paste0(
        "Alternative-preprocessing H01 crosswalk differs from the shared ",
...
h01_alternative_preprocessing_contract <- function(
  crosswalk,
  main_contract
) {
  selected <- h01_alternative_preprocessing_selected_crosswalk(
    crosswalk,
    main_contract
  )
  ...
  expected_variants <- c(
    dose_time_sensitive_corrected_medi = "Uncorrected alternative-preprocessing dose",
  ...
  expected_definitions <- c(
    dose_time_sensitive_corrected_medi = paste(
      "Alternative-preprocessing melEDI dose without the new",
  ...
  if (
    ...
    h01_abort(
      paste0(
        "Alternative-preprocessing dose or MDER semantics are not explicit, ",
        "or another metric variant changed"
      )
    )
  }
  fields <- h01_implementation_contract_columns()
  if (!identical(main_contract[fields], scenario_contract[fields])) {
    h01_abort("Primary and alternative preprocessing use different metric specifications")
  }
  list(contract = scenario_contract, crosswalk = selected)
...
h01_alternative_preprocessing_timing_check <- function(before, after) {
  timing <- h01_alternative_preprocessing_timing_metrics()
  selected <- before |>
    dplyr::filter(.data$metric_id %in% timing) |>
    dplyr::rename(input_value = "alternative_preprocessing_value") |>
    dplyr::left_join(
      after |>
        dplyr::filter(.data$metric_id %in% timing) |>
        dplyr::select(
  ...
  selected |>
    ...
    dplyr::mutate(
      data_scenario_id = h01_alternative_preprocessing_scenario_id(),
      model_implementation_id = h01_alternative_preprocessing_model_implementation_id(),
...
h01_alternative_preprocessing_contract_equivalence <- function(
  main_contract,
  scenario_contract,
  selected_crosswalk
) {
...
h01_alternative_preprocessing_validate_source_ids <- function(
  participant_day,
  participant
) {
  scenario_id <- h01_alternative_preprocessing_scenario_id()
  implementation_id <-
    h01_alternative_preprocessing_model_implementation_id()
  for (data in list(participant_day, participant)) {
    assert_columns(
      ...
      object = "alternative-preprocessing H01 source"
    )
    ...
    if (
      ...
      h01_abort(
        "Alternative-preprocessing source IDs differ from the declared scenario"
      )
    }
  }
  invisible(TRUE)
...
h01_alternative_preprocessing_assert_complete_metric_grid <- function(
  data,
  contract,
  object
) {
...
h01_alternative_preprocessing_rename_metric_fields <- function(data, contract) {
  for (index in seq_len(nrow(contract))) {
    ...
    if (!metric_id %in% names(data)) {
      h01_abort(
        "Alternative-preprocessing wide data are missing metric `%s`",
        metric_id
      )
    }
...
h01_build_alternative_preprocessing_inputs <- function(root = project_root()) {
  root <- normalizePath(root, winslash = "/", mustWork = TRUE)
  paths <- h01_alternative_preprocessing_output_paths(root, root)
  required <- c(
  ...
  if (length(missing) > 0L) {
    h01_abort(
      "Alternative-preprocessing H01 adapter is missing input(s): %s",
  ...
  h01_alternative_preprocessing_validate_source_ids(
  ...
  contract_result <- h01_alternative_preprocessing_contract(
    crosswalk,
    main_contract
  )
  ...
  assert_unique_key(
    ...
    object = "alternative-preprocessing H01 participant-day metrics"
  )
  assert_unique_key(
    participant_selected,
    c("position", "site", "Id", "metric_id"),
    object = "alternative-preprocessing H01 participant metrics"
  )
  h01_alternative_preprocessing_assert_complete_metric_grid(
    day_selected,
    contract[contract$analysis_unit == "participant_day", , drop = FALSE],
    "Alternative-preprocessing participant-day metrics"
  )
  h01_alternative_preprocessing_assert_complete_metric_grid(
    participant_selected,
    contract[contract$analysis_unit == "participant", , drop = FALSE],
    "Alternative-preprocessing participant metrics"
  )
  ...
  day_long <- day_selected |>
    ...
    dplyr::mutate(
      value = ifelse(
        .data$metric_id %in%
          h01_alternative_preprocessing_timing_metrics(),
        h01_clock_hour_to_minute(.data$alternative_preprocessing_value),
  ...
  if (
    ...
    h01_abort(
      "Alternative-preprocessing daily metrics have invalid analysis units"
    )
  }
  timing_check <- h01_alternative_preprocessing_timing_check(
    day_selected,
    day_long
  )
  l10_audit <- timing_check |>
    dplyr::filter(.data$metric_id == "l10_midpoint")
  non_l10_audit <- timing_check |>
    dplyr::filter(.data$metric_id != "l10_midpoint")
  raw_timing <- day_selected |>
    dplyr::filter(
      .data$metric_id %in% h01_alternative_preprocessing_timing_metrics()
    )
  converted_timing <- day_long |>
    dplyr::filter(
      .data$metric_id %in% h01_alternative_preprocessing_timing_metrics()
    )
  finite_timing <- is.finite(raw_timing$alternative_preprocessing_value)
  reversible_difference <- abs(
    (converted_timing$value[finite_timing] /
      60 -
      raw_timing$alternative_preprocessing_value[finite_timing] +
      12) %%
      24 -
      12
  )
  if (
    sum(l10_audit$negative_input_values) != 62L ||
      ...
      l10_audit$negative_input_values[
        ...
        33L ||
      sum(timing_check$wrapped_values) != 62L ||
      any(non_l10_audit$input_values_outside_0_24 != 0L) ||
      any(
        raw_timing$alternative_preprocessing_value >= 24,
        na.rm = TRUE
      ) ||
      !identical(
        is.finite(raw_timing$alternative_preprocessing_value),
    ...
    h01_abort(
      "Alternative-preprocessing timing conversion differs from its specified units"
    )
  }
  ...
  day_wide <- h01_alternative_preprocessing_rename_metric_fields(
    day_wide,
    contract[contract$analysis_unit == "participant_day", , drop = FALSE]
  )
  ...
  day_wide <- day_wide |>
    ...
    dplyr::mutate(
      profile_variant = h01_alternative_preprocessing_scenario_id(),
      measurement_construct = h01_alternative_preprocessing_measurement_construct(
        .data$position
      ),
      prepared_record_support_available = FALSE,
      prepared_record_support_unavailability_reason = h01_alternative_preprocessing_support_reason(),
  ...
  if (
    ...
    h01_abort(
      "Alternative-preprocessing H01 participant-days lack site context"
    )
  }
  ...
  participant_long <- participant_selected |>
    ...
    dplyr::rename(value = "alternative_preprocessing_value")
  if (
    anyNA(participant_long$analysis_unit) ||
      any(participant_long$analysis_unit != "participant")
  ) {
    h01_abort(
      "Alternative-preprocessing participant metrics have invalid analysis units"
    )
  }
  ...
  participant_wide <- h01_alternative_preprocessing_rename_metric_fields(
    participant_wide,
    contract[contract$analysis_unit == "participant", , drop = FALSE]
  )
  ...
  combined_long <- dplyr::bind_rows(
    ...
    dplyr::mutate(
      ...
      failure_reason = ifelse(
        .data$estimable,
        NA_character_,
        h01_alternative_preprocessing_missing_metric_reason()
      ),
      .after = "local_date"
    )
  ...
  metric_support <- combined_long |>
    dplyr::transmute(
      ...
      metric_support_unavailability_reason = h01_alternative_preprocessing_support_reason(),
      valid_minutes = NA_real_,
      expected_minutes = NA_real_
    )
  ...
  equivalence <- h01_alternative_preprocessing_contract_equivalence(
    main_contract,
    contract,
    selected_crosswalk
  )
  list(
    ...
    timing_conversion_check = timing_check
  )
}
