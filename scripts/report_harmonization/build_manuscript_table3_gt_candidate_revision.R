...
label_line <- function(label, value, bold = FALSE) {
  line <- paste0(
  ...
  if (bold) paste0("<strong>", line, "</strong>") else line
}

fdr_reader_value <- function(value) {
  value <- sub("^FDR-supported", "supported", value)
  sub("^Not retained", "not supported", value)
}
...
for (row in body_rows) {
  ...
  records[[length(records) + 1L]] <- data.frame(
    ...
    descriptive = paste0(
      ...
      label_line("Participants", values[[6L]]),
      label_line("Days", values[[7L]])
    ),
    distribution = image_html,
    overall_site = paste0(
      label_line("FDR", fdr_reader_value(values[[9L]]), grepl("^FDR-supported", values[[9L]])),
      label_line("Part R-squared", values[[10L]])
    ),
    photoperiod = paste0(
      label_line("Estimate", values[[11L]]),
      label_line("FDR", fdr_reader_value(values[[12L]]), grepl("^FDR-supported", values[[12L]])),
...
if (
  nrow(candidate_rows) != 17L ||
    !identical(unique(candidate_rows$metric_family), expected_source_groups) ||
...
# Harmonize Table 3 to the accepted descriptive-table display order. This is
# a row-order-only presentation change over the pinned 17-row source.
descriptive_metric_order <- c(
...
row_order <- match(descriptive_metric_order, candidate_rows$metric)
if (anyNA(row_order) || anyDuplicated(row_order)) {
  stop("Could not align Table 3 to the descriptive metric order.")
}
candidate_rows <- candidate_rows[row_order, , drop = FALSE]
if (!identical(unique(candidate_rows$metric_family), descriptive_group_order)) {
  stop("The harmonized Table 3 group order does not match the descriptive table.")
}
...
candidate_gt <- gt(
  ...
  tab_source_note(
    source_note = md(paste0(
      "Overall distributions are median (interquartile range). Participants is the ",
...
candidate_css <- paste0(
  ...
  ".table3-candidate-scroll .metric-density-thumb{display:block;width:145px;height:82px;object-fit:contain;margin:auto;}",
  ...
  ".table3-candidate-scroll tbody tr:not(:has(.gt_group_heading)) td,",
  ".table3-candidate-scroll tbody tr:not(:has(.gt_group_heading)) th{height:90px;max-height:94px;vertical-align:middle;}",
  ".table3-candidate-scroll .gt_group_heading{height:24px!important;padding-top:2px!important;padding-bottom:2px!important;}",
  ".table3-candidate-scroll .gt_stub{overflow-wrap:normal;word-break:normal;line-height:1.15;}",
  "</style>"
)
candidate_html <- paste(
  paste0(
    '<div id="tbl-plan-h01-metric-synthesis-candidate" ',
    'class="table3-candidate-scroll" role="region" tabindex="0" ',
    'aria-label="Near-eye personal light-exposure metrics and their ',
    'geographic and photoperiod context">'
  ),
  candidate_css,
  raw_table,
  "</div>",
...
candidate_fragment <- paste(
  "<!-- Native gt candidate for manuscript Table 3. -->",
  "```{=html}",
  candidate_html,
  "```",
  sep = "\n"
