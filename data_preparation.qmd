...
sleepdiary_adj <-
  ...
  map(\(x) x |>
        ...
        sleep_int2Brown(Brown.day = "wake", Brown.evening = "pre-sleep", Brown.night = "sleep") |>
...
In this step, we will remove non-wear instances that are neither marked as sleep, nor fall into a sleep window (sleepdiary). We will also add photoperiod, and remove instances ≥ 10^5 lx melanopic EDI.
...
metric_chest_participantday <-
  ...
  summarize(
    ...
    bright_dark_period(log_zero_inflated(MEDI), Datetime, "darkest", "5 hours", loop = TRUE, na.rm = TRUE, as.df = TRUE),
...
metric_chest_participanthour <- 
  ...
  aggregate_Datetime(
    "1 hour",
    type = "floor",
    numeric.handler = \(x) x |> log_zero_inflated() |> mean(na.rm = TRUE) |> exp_zero_inflated()
  )
...
save(metric_chest_participanthour, 
     metric_chest_participant, 
     metric_chest_participantday, 
     file = "data/metrics_chest.RData")
save(metric_glasses_participanthour, 
     metric_glasses_participant, 
     metric_glasses_participantday, 
     file = "data/metrics_glasses.RData")
```
