...
1. A one-minute set of light exposure data, both for the eye-level and chest-level wearing position where
  - melanopic EDI values at or above the manufacturer's reported upper operating boundary of 100,000 lx are flagged and set to `NA` for analysis, while the pre-mask values are retained in `MEDI_raw` [@condor2023actlumus]
  - the operating-boundary rule is not applied to photopic illuminance (`LIGHT`)
  - logged `off` periods outside diary-defined sleep are masked in both
    analytical light channels; raw values are retained
...
3. A further processed set (from 2.) where personal light exposure metrics are calculated
  ...
  - level-based:
      ...
      - `L10mean`: geometric mean of melanopic EDI during the 10 darkest hours of the day (lx)
  - duration-based:  
  ...
  - timing-based:
      ...
      - `L10`: midpoint of the 10 darkest hours (HH:MM)
...
In this step, we remove non-wear instances that are neither marked as sleep nor fall within a sleep window defined by the sleep diary. We also add photoperiod and flag one-minute melanopic EDI values at or above the manufacturer's reported upper operating boundary of 100,000 lx. These values are set to missing for analysis, while the pre-mask values are retained in `MEDI_raw`. This device-range rule is not applied to photopic illuminance (`LIGHT`).

```{r}
light_chest_expanded <- 
light_chest_expanded |> 
  imap(\(x, idx) x |> 
    mutate(MEDI_raw = MEDI,
           LIGHT_raw = LIGHT,
           invalid_nonwear = wear == "off" & (State.Brown != "sleep" | is.na(State.Brown)),
           medi_saturated = is.finite(MEDI_raw) & MEDI_raw >= 100000,
           MEDI = replace_when(MEDI,
                               invalid_nonwear ~ NA,
                               medi_saturated ~ NA
                               ),
           LIGHT = replace_when(LIGHT,
                                invalid_nonwear ~ NA
                                )) |>
