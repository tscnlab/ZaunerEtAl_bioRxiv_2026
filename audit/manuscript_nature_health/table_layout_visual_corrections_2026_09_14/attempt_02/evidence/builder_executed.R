...
style<-paste(c("html { background:#eef1f4; color:#17202a; }",
 ...
 "body:has(#physical-proof:checked) #capture-rectangle { zoom:var(--physical-proof-scale); }",
 ".proof-control { display:block; margin:0 0 12px; }",
...
for(i in seq_len(nrow(pages))) {
 ...
 '<p id="scroll-help">If the table is wider than this window, scroll horizontally inside its bordered area. Keyboard users can focus that area and use the arrow keys.</p>',
 '<label class="proof-control"><input id="physical-proof" type="checkbox"> Show inherited print-width proof (',if(p$key=="supp_table_s2")'15.55' else '10.55',' inches at 96 CSS pixels per inch; not final Word output)</label></header>',
 '<div class="table-scroll" style="--physical-proof-scale:',sprintf("%.12f",(if(p$key=="supp_table_s2")15.55 else 10.55)*96/(if(p$key=="supp_table_s2")sum(s2width) else if(p$key=="supp_table_s7")896 else 1320)),'" role="region" tabindex="0" aria-label="',esc(title),': horizontally scrollable table" aria-describedby="scroll-help"><main id="capture-rectangle">',content,'</main></div>',
