...
parts_raw<-read.csv(file.path(out,"maps/complete_table_part_map.csv"),colClasses="character",check.names=FALSE)
oldparts_raw<-read.csv(file.path(base,"maps/complete_table_part_map.csv"),colClasses="character",check.names=FALSE)
for(i in which(!(oldparts$key %in% c("main_table_2","supp_table_s2")) & !(oldparts$key=="supp_table_s7" & oldparts$part>=3L)))check(paste0("UNCHANGED_REUSE_",i),identical(parts_raw[i,],oldparts_raw[i,]))
