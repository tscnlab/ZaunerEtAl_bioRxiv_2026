...
cell_text_tokens <- function(node) {
  cells <- xml_find_all(node, ".//th|.//td")
  unlist(
    lapply(
      cells,
      function(cell) {
        text_nodes <- xml_find_all(cell, ".//text()[normalize-space()]")
        trimws(gsub("[[:space:]]+", " ", xml_text(text_nodes)))
      }
    ),
    use.names = FALSE
  )
...
normalize_smart_typography <- function(text) {
...
coordination_contract_root <- file.path(
  project_root,
...
if (phase == "html") {
  ...
  historical_document <- read_html(file.path(
    owner_root,
  ...
  for (index in seq_len(nrow(table_contract))) {
    ...
    historical_node <- xml_find_first(
      historical_document,
      paste0("//*[@id='", endpoint, "']")
    )
    source_cells <- vapply(
    ...
    historical_cells <- vapply(
      xml_find_all(historical_node, ".//th|.//td"),
      normalize_text,
      character(1)
    )
    source_tokens <- cell_text_tokens(source_node)
    rendered_tokens <- cell_text_tokens(rendered_node)
    table_rows[[index]] <- data.frame(
      ...
      source_text_tokens = length(source_tokens),
      rendered_text_tokens = length(rendered_tokens),
      cell_text_exact = identical(source_tokens, rendered_tokens),
      historical_rendered_cells_exact = identical(
        historical_cells,
        rendered_cells
      ),
      stringsAsFactors = FALSE
  ...
  write.csv(
    table_rows,
    file.path(verification_output_root, "qa/table_endpoint_checks.csv"),
    row.names = FALSE,
    na = ""
  )
  add_check(
  ...
  prior_svg_candidate_path <- file.path(
  ...
  caption_exact <- vapply(
    ...
    function(index) {
      endpoint <- c("fig-s6", "fig-s12")[[index]]
      rendered_caption <- normalize_text(xml_find_first(
        document,
        paste0("//*[@id='", endpoint, "']//figcaption")
      ))
      identical(
        normalize_smart_typography(rendered_caption),
        normalize_smart_typography(inserted_caption_text[[index]])
      )
    },
    logical(1)
  )
...
output_path <- file.path(verification_output_root, "qa", paste0(phase, "_checks.csv"))
