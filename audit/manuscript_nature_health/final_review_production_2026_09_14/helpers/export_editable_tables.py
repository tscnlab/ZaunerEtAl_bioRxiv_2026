...
WIDTHS = {
    ...
    "Table_2": [176,254,230,180,180,200,100],
    "Table_3": [130, 190, 140, 110, 150, 235, 265],
    "Table_S1": [1.75, 3.65, 1.35],
    "Table_S2": [210,86,138,146,146,154,138,146,138,146,138,130,120,252],
...
def add_run(paragraph, text, style):
    ...
    if style.get("complete_mean_sd_spacing_twips") is not None:
        element_property(run._r.get_or_add_rPr(), "w:spacing", {"w:val":style["complete_mean_sd_spacing_twips"]})
    if style.get("colour"):
...
def render_inline(element, paragraph, style, width, source_directory):
    ...
    if (current.get("s2_numeric") and styles.get("white-space") == "nowrap"
        ...
        current["nowrap"] = True
        current["complete_mean_sd_spacing_twips"] = -10
...
def export_one(source_table, source_path, reference, outdir, index, s2_numeric_reduction_pt=0):
    ...
    for r,c,rs,cs,source in records:
        ...
        if key=="Table_S2" and not heading and not group and cs==1 and 2<=c<=11:
            ...
            tight_margins=element_property(cell._tc.get_or_add_tcPr(),"w:tcMar",{})
            for edge in ("left","right"):
                element_property(tight_margins,"w:"+edge,{"w:w":10,"w:type":"dxa"})
    ...
    return {
        ...
        **({"complete_mean_sd_nowrap_units":s2_nowrap_spans_verified,
            ...
            "numeric_reduction_pt":s2_numeric_reduction_pt,
            "numeric_side_padding_pt":0.5,
            "complete_mean_sd_character_spacing_pt":-0.5} if key=="Table_S2" else {}),
    }
...
def main():
    ...
    parser.add_argument("--s2-numeric-reduction-pt",type=int,choices=(0,),default=0)
