...
target <- file.path(w, "render_html_round2/ZaunerEtAl2026_NatHealth_phase3_brown.html")
...
check("no_duplicate_ids_including_styles", length(duplicate_ids) == 0L)
...
old <- read.csv(file.path(p, "evidence/round2_inputs.csv"))
check("round2_input_hashes_unchanged", identical(unname(vapply(old$path, sha, character(1))), old$sha256))
write.csv(cells, file.path(p, "evidence/html_ordered_cells_round2.csv"), row.names = FALSE)
write.csv(protected, file.path(p, "evidence/html_407_cell_preservation_round2.csv"), row.names = FALSE)
result <- do.call(rbind, checks)
write.csv(result, file.path(p, "evidence/html_semantic_checks_round2.csv"), row.names = FALSE)
writeLines(capture.output(sessionInfo()), file.path(p, "evidence/html_verification_R_session_round2.txt"))
...
write_json(rec, file.path(p, "evidence/html_semantic_result_round2.json"), auto_unbox = TRUE, pretty = TRUE)
if (all(result$pass)) write_json(rec, file.path(p, "evidence/html_semantic_pass_round2.json"), auto_unbox = TRUE, pretty = TRUE)
