...
def main():
    ...
    checks = {
        ...
        "svg_part_count_exact": len(added) + len(retained) == len(authority["accepted_figures"]) == 23
            and len(set(added + retained)) == 23
            and len([name for name in parts if name.startswith("word/media/") and name.endswith(".svg")]) == 23,
        "drawing_contract_exact": len(document.xpath(".//wp:inline", namespaces=NS)) == 54
            and sum(len(record["drawings"]) for record in records) == 24,
    ...
    report = {
        "status": "Order 009 complete manuscript-plus-SI candidate, not promoted.",
