...
def plan(stage,r):
    ...
    commands={
      ...
      'native':[PY,str(h/'export_editable_tables.py'),str(W/f'render_html_round{r}'/Q.replace('.qmd','.html')),str(P/'project/assets/reference.docx'),str(P/'editable_tables'), '--only','Table_2','Table_S2'],
      'assembly':[PY,str(h/'prepare_word_manuscript.py'),str(W/f'render_docx_round{r}'/Q.replace('.qmd','.docx')),str(m/'word_table_png_manifest.json'),str(m/'word_figure_svg_manifest.json'),str(d/f'manuscript_assembled_round{r}.docx'),'--part-count-contract',str(m/'part_count_contract.json')],
    ...
    if stage.startswith('qa_'):
        label=stage[3:]
        source=d/f'Nature_Health_manuscript_round{r}.docx' if label=='main' else P/'editable_tables'/f'{label}.docx'
