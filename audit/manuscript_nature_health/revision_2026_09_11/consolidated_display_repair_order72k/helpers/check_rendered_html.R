...
paths <- c(selection=file.path(root,"audit/manuscript_nature_health/consolidated_display_repair_2026_09_11/integration_candidate_order72k/project/render_attempt1/selection.html"),main=file.path(out,"project/render_html_attempt1/ZaunerEtAl2026_NatHealth_phase3_brown.html"))
...
write.csv(results,file.path(out,"html_initial_structure_checks.csv"),row.names=FALSE)
jsonlite::write_json(details,file.path(out,"html_initial_structure_details.json"),auto_unbox=TRUE,pretty=TRUE)
