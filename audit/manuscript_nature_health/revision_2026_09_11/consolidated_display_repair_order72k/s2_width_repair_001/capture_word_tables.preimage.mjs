          ...
          for (const cell of table.querySelectorAll("tbody tr > :nth-child(n+3):nth-child(-n+12)")) {
            ...
            while (walker.nextNode()) {
              ...
              for (const rect of range.getClientRects()) {
                if (rect.left < cellBox.left - 0.5 || rect.right > cellBox.right + 0.5) {
                  numericalTextOverflow.push({text:walker.currentNode.textContent,left:rect.left,right:rect.right,cellLeft:cellBox.left,cellRight:cellBox.right});
                }
              }
            }
          }
        }
        if (numericalTextOverflow.length) throw new Error(`S2 numerical text clipping: ${JSON.stringify(numericalTextOverflow)}`);
        return {fonts, images, numericalTextOverflow, bodyCells: Array.from(table.querySelectorAll("tbody tr")).map((row) => Array.from(row.children).map((cell) => cell.textContent))};
