    ...
    for record in authority["accepted_figures"]:
        ...
        for drawing in drawings:
            ...
            if extension_list is not None:
                existing_svg = extension_list.xpath("./a:ext[@uri=$uri]/asvg:svgBlip", namespaces=NS, uri=SVG_EXTENSION)
                if len(existing_svg) != 1 or len(extension_list) != 1:
                    raise SystemExit(f"Unexpected pre-existing image extension for {label}")
                ext_rel = existing_svg[0].get("{" + NS["r"] + "}embed")
                original_target = posixpath.normpath("word/" + rel_lookup[ext_rel].get("Target"))
                if digest(parts[original_target]) != record["sha256"]:
                    raise SystemExit(f"Pre-existing SVG bytes differ for {label}")
            else:
                ...
                new_extensions.add(drawing.find("wp:docPr", NS).get("id"))
