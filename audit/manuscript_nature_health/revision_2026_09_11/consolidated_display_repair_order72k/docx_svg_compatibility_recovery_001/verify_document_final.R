...
stopifnot(!file.exists(file.path(record, "final_document_checks.csv")))
...
heading1_style_id <- attr_ns(style_nodes[which(tolower(style_names) == "heading 1")], "w:styleId")
...
write.csv(rows,file.path(record,"final_drawing_payload_identity.csv"),row.names=FALSE)
write.csv(png_rows,file.path(record,"final_table_part_geometry.csv"),row.names=FALSE)
write.csv(section_rows,file.path(record,"final_section_geometry.csv"),row.names=FALSE)
write.csv(checks,file.path(record,"final_document_checks.csv"),row.names=FALSE)
writeLines(c("R4.6.1 independent package, payload, geometry, text and link checks only; no scientific calculation.",
 capture.output(sessionInfo())),file.path(record,"final_document_verification_session.txt"))
