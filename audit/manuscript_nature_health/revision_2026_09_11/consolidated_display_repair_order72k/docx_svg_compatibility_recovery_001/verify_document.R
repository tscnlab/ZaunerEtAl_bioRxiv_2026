...
stopifnot(!file.exists(file.path(record, "document_checks.csv")))
...
assembly_report <- jsonlite::fromJSON(file.path(record, "assembly_execution_receipt.json"))$report
...
add("all_table_aspect_ratios_within_two_EMU_rounding", all(png_rows$aspect_error <= 2))
...
heading1 <- paragraphs[vapply(paragraphs, function(p) {
 s <- find(p, "./w:pPr/w:pStyle"); length(s) == 1L && attr_ns(s,"w:val") == "Heading1"
}, logical(1))]
...
before_names <- attr_ns(find(raw_xml, ".//w:bookmarkStart"), "w:name")
added <- assembly_report$display_bookmarks$added
add("exact_22_new_display_bookmarks", length(setdiff(start_names,before_names)) == 22L && setequal(setdiff(start_names,before_names),added$name))
...
write.csv(rows,file.path(record,"drawing_payload_identity.csv"),row.names=FALSE)
write.csv(png_rows,file.path(record,"table_part_geometry.csv"),row.names=FALSE)
write.csv(section_rows,file.path(record,"section_geometry.csv"),row.names=FALSE)
write.csv(checks,file.path(record,"document_checks.csv"),row.names=FALSE)
writeLines(c("R4.6.1 independent package, payload, geometry, text and link checks only; no scientific calculation.",
 capture.output(sessionInfo())),file.path(record,"document_verification_session.txt"))
