...
canonical_row <- function(r) {
 copy <- read_xml(as.character(r))
 xml_attrs_to_drop <- c("data-whole-mean-sd","data-numeric-column","data-distribution-cell")
 for(a in xml_attrs_to_drop) xml_set_attr(xml_find_all(copy,paste0("descendant-or-self::*[@",a,"]")),a,NA_character_)
 sub("[\r\n]+$","",as.character(copy))
}
...
extract_abstract<-function(x)sub("^.*?# Abstract\\n\\n(.*?)\\n\\n# Introduction.*$","\\1",x,perl=TRUE)
...
include<-sub("^.*\\{\\{< include ([^>]+) >\\}\\}.*$","\\1",b1,perl=TRUE)
...
for(i in which(parts$key!="main_table_2" & parts$key!="supp_table_s2" & !(parts$key=="supp_table_s7" & parts$part>=3))) {
 ...
 a<-parts[i,,drop=FALSE];b<-parts0[j,,drop=FALSE];rownames(a)<-rownames(b)<-NULL
