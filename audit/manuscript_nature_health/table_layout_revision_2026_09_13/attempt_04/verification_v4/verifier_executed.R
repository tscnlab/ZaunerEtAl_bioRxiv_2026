...
# Only the allowed main Table 2 block and its directly dependent SI reference change.
...
siold<-rawtext(file.path(accepted,"source/supplementary_information_outline.qmd"))
sinew<-rawtext(file.path(out,"source/supplementary_information_outline.qmd"))
oldref<-"Main Table 2 includes that sensitivity."
newref<-"The coverage sensitivity is reported in the Results and Methods."
check("SI_DEPENDENT_REFERENCE_ONLY",identical(sub(newref,oldref,sinew,fixed=TRUE),siold))
check("SI_REFERENCE_RESOLVED",!grepl(oldref,sinew,fixed=TRUE) && grepl(newref,sinew,fixed=TRUE))
check("SI_REFERENCE_TARGETS_RETAINED",grepl("At least 80% coverage",mainnew,fixed=TRUE) &&
 grepl("the coverage sensitivity required",mainnew,fixed=TRUE))
cum0<-read.csv(file.path(accepted,"verification/cumulative_passage_changes.csv"),check.names=FALSE)
cum1<-read.csv(file.path(out,"evidence/cumulative_passage_changes.csv"),check.names=FALSE)
ki<-match(c("Results / Table 2","Supplement / fig-s4"),cum0$position)
check("CUMULATIVE_20_POSITIONS",nrow(cum1)==20L && identical(cum1$position,cum0$position))
check("CUMULATIVE_OLD_TEXT_EXACT",identical(cum1$old_text,cum0$old_text))
check("CUMULATIVE_OTHER_ROWS_EXACT",identical(cum1[-ki,],cum0[-ki,]))
check("CUMULATIVE_TABLE2_MATCH",identical(cum1$new_text[ki[1]],b1))
check("CUMULATIVE_FIG_S4_MATCH",identical(cum1$new_text[ki[2]],sub(oldref,newref,cum0$new_text[ki[2]],fixed=TRUE)))
