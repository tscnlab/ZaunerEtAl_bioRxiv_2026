...
sipath <- pin(file.path(accepted,"source/supplementary_information_outline.qmd"),"90c215ae0597547cffe2f0498e888ffb190e06c65bbbf60cda7b65cc69a7c199","accepted supplementary preimage; one directly dependent Table 2 reference")
...
si <- rawtext(sipath)
oldref <- "Main Table 2 includes that sensitivity."
newref <- "The coverage sensitivity is reported in the Results and Methods."
stopifnot(length(gregexpr(oldref,si,fixed=TRUE)[[1]])==1L,grepl(oldref,si,fixed=TRUE),
 !grepl(newref,si,fixed=TRUE),!grepl(oldref,main,fixed=TRUE))
newsi <- sub(oldref,newref,si,fixed=TRUE)
stopifnot(identical(sub(newref,oldref,newsi,fixed=TRUE),si))
put(newsi,file.path(out,"source/supplementary_information_outline.qmd"))
put(oldblock,file.path(out,"evidence/Table_2_block_before.txt"))
put(newblock,file.path(out,"evidence/Table_2_block_after.txt"))
write.csv(data.frame(position=c("Table 2 block","Supplement / fig-s4: dependent sensitivity reference"),
 old_text=c(oldblock,oldref),new_text=c(newblock,newref),
 reason=c("Author-approved single primary-sample table; caption reconciled, all other main source bytes preserved",
 "The primary-only Table 2 no longer contains the sensitivity; redirect to its unchanged Results/Methods statements")),
 file.path(out,"evidence/changed_passages.csv"),row.names=FALSE)

# Preserve the cumulative author-facing old/new record, updating only these two entries.
cumpath <- pin(file.path(accepted,"verification/cumulative_passage_changes.csv"),role="accepted cumulative passage record")
cum <- read.csv(cumpath,check.names=FALSE)
cum_before <- cum
k <- match(c("Results / Table 2","Supplement / fig-s4"),cum$position)
stopifnot(!anyNA(k),nrow(cum)==20L)
escaped_newlines <- function(x) gsub("\n","\\n",x,fixed=TRUE)
stopifnot(identical(cum$new_text[k[1]],escaped_newlines(oldblock)),grepl(oldref,cum$new_text[k[2]],fixed=TRUE))
cum$new_text[k[1]] <- escaped_newlines(newblock)
cum$new_text[k[2]] <- sub(oldref,newref,cum$new_text[k[2]],fixed=TRUE)
cum$kind[k[1]] <- "single primary-sample table include and caption"
cum$basis[k[1]] <- "Accepted current Brown results and author source/static order 005; primary sample only"
cum$basis[k[2]] <- paste0(cum$basis[k[2]],"; order 005 redirects its dependent sensitivity reference to Results/Methods")
stopifnot(identical(cum$old_text,cum_before$old_text),identical(cum[-k,],cum_before[-k,]))
write.csv(cum,file.path(out,"evidence/cumulative_passage_changes.csv"),row.names=FALSE)
mdcell <- function(x) {
 x <- gsub("|","\\|",x,fixed=TRUE)
 x <- gsub("\\n","<br>",x,fixed=TRUE)
 gsub("\n","<br>",x,fixed=TRUE)
}
cum_md <- c("# Cumulative manuscript passage changes", "",
 "Candidate source record. These are the 20 cumulative changed positions, not evidence that final Word or website production has occurred.", "",
 "The accepted prior record is preserved; only Table 2 and its directly dependent Supplementary Figure S4 reference are updated by order 005.", "",
 "| Position | Old text | New text |", "| --- | --- | --- |",
 paste0("| ",mdcell(cum$position)," | ",mdcell(cum$old_text)," | ",mdcell(cum$new_text)," |"))
put(paste0(paste(cum_md,collapse="\n"),"\n"),file.path(out,"evidence/cumulative_passage_changes.md"))
