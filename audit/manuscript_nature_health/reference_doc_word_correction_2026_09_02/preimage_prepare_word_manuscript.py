...
def set_final_portrait_section(document: Document):
    section_properties = document.element.body.find(qn("w:sectPr"))
    if section_properties is None:
        raise RuntimeError("DOCX body has no final section properties")
    ...
    page_size.set(qn("w:w"), str(round(8.5 * 1440)))
    page_size.set(qn("w:h"), str(round(11 * 1440)))
    page_size.attrib.pop(qn("w:orient"), None)

    margins = section_properties.find(qn("w:pgMar"))
    if margins is None:
        margins = OxmlElement("w:pgMar")
...
def main() -> None:
    ...
    set_final_portrait_section(document)
