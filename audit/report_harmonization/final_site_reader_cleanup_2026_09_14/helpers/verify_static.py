...
def verify(mode,tag):
    ...
    csvout(E/(tag+'_complete_reference_exceptions.csv'),errors)
    csvout(E/(tag+'_complete_reference_scan.csv'),refs)
    ...
    classfile=CONTROL/'order017_inherited_classification/five_inherited_links.csv'
    check('Sealed five-link classification',exact(classfile,'3866ed79500ad9a3512b1fe8abc1bb24ff7a4f517e629145f3d10bc55c16a098',1503))
    five=readcsv(classfile);additional=[]
    for r in five:
        check('Exact five-link route pre/post binding',r['path'] not in routes and baseline[r['path']]['sha256']==r['pre_sha256'] and actual[r['path']]['sha256']==r['candidate_sha256'],r['path'])
        missing_assembly=r['classification']=='inherited_missing_assembly_page'
        additional.append(dict(owner=r['path'],reference=r['reference'],kind='a@href',target='notebooks/assemble_artifacts.html' if missing_assembly else 'notebooks/hypotheses/H03.html',status='MISSING' if missing_assembly else 'MISSING_FRAGMENT'))
    check('Only four pinned fonts plus five exact old non-reader links',len(errors)==9 and {sig(r) for r in errors}=={sig(r) for r in accepted+additional},errors)
    ...
    result=dict(utc=utc(),status='PASS',mode=mode,checks=len(checks),public_files=899,html_files=44,public_replacements=45,retirements=15,unchanged=854,routes=36,search_records=938,sitemap_entries=38,downloads=20,reference_rows=len(refs),inherited_font_exceptions=4,inherited_unregistered_links=5,inherited_semantic_failure_occurrences=sum(r['occurrences'] for r in semantic_rows))
