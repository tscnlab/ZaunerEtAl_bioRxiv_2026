# Order017 copied accepted Writer-content predicates. No results are recomputed.
...
candidate <- Sys.getenv("ORDER017_SITE_ROOT", file.path(out, "candidate_build"))
options(warn=2)
checks <- list()
...
check("Authorized editorial utility removed",length(xml_find_all(s,"//table[contains(@class,'site-change-table')]"))==0L && !file.exists(file.path(candidate,"manuscript_changes.csv")))
write.csv(do.call(rbind,checks),file.path(evidence,"content_reconciliation_R.csv"),row.names=FALSE)
capture.output(sessionInfo(),file=file.path(evidence,"content_R_sessionInfo.txt"))
writeLines(c("No scientific estimates were calculated, reinterpreted or refitted.",paste("Command: Rscript --vanilla",file.path(out,"helpers/verify_content.R")),paste("Accepted HTML:",accepted),paste("Accepted SHA256:",digest(file=accepted,algo="sha256")),paste("Verified site:",candidate),paste("Verified index SHA256:",digest(file=file.path(candidate,"index.html"),algo="sha256"))),file.path(evidence,"content_R_provenance.txt"))
