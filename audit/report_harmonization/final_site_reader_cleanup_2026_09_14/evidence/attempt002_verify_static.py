...
def verify(mode,tag):
    ...
    for route in htmlroutes:
        ...
        if route in ['index.html','supplementary_information.html']: check('No entry duplicate IDs',not dup,route)
        for n in d.xpath('//*[@aria-labelledby]|//*[@aria-describedby]|//*[@for]'):
            for attr in ['aria-labelledby','aria-describedby','for']:
                for identity in n.get(attr,'').split():
                    ...
                    check('No unresolved semantic IDREF',ok,(route,attr,identity))
            ...
            for n in t.xpath('.//*[@headers]'):
                for identity in n.get('headers').split():
                    if route in ['index.html','supplementary_information.html']: check('Unique entry table-scoped header',scoped[identity]==1,(route,identity))
                        ...
                        check('Non-entry header exists with exact inherited ID multiplicity',scoped[identity]>=1 and scoped[identity]==oldscoped[identity],(route,identity,scoped[identity]))
    ...
    for filename,data,fields in [('static_checks',checks,None),('ids',idrows,None),('semantic_idrefs',idrefs,None),('references',refs,None),('reference_exceptions',errors,None),('external_references',external,None),('corpus_delta',deltas,None)]:csvout(E/(tag+'_'+filename+'.csv'),data,fields)
