...
out <- file.path(root, "audit/report_harmonization/report018_order72k_non_s5_integration_dispatch/svg_stop_independent_001")
...
check("all three and only three raw drawings are exact accepted main SVGs", nrow(drawings)==3L && nrow(main)==3L && identical(main$accepted_label,paste("Main Figure",1:3)))
...
check("image relationships are exactly the three SVGs", setequal(rel$id[grepl("/image$",rel$type)],main$svg_embed))
