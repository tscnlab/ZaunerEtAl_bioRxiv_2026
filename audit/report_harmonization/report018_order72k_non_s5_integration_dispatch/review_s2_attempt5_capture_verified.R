...
dimension_rows <- lapply(seq_along(entries), function(i) {
  ...
  requested_preview <- as.integer(round(c(width,height) * 96))
  expected_preview <- as.integer(round(original * min(requested_preview / original)))
  stopifnot(identical(original, c(2980L, c(1738L,1452L,1480L)[i])),
    ...
    identical(requested_preview, as.integer(c(geometry[[i]]$previewWidth, geometry[[i]]$previewHeight))),
    identical(small, expected_preview))
  data.frame(part = i, png = entries[[i]]$path, png_sha256 = sha(entries[[i]]$path),
    ...
    requested_preview_width = requested_preview[1], requested_preview_height = requested_preview[2],
    actual_preview_width = small[1], actual_preview_height = small[2])
})
...
writeLines(c("Command: RENV_CONFIG_AUTOLOADER_ENABLED=FALSE Rscript --vanilla audit/report_harmonization/report018_order72k_non_s5_integration_dispatch/review_s2_attempt5_capture_verified.R",
  ...
  "The original checker is preserved. It compared final raster dimensions with the requested resize box. This version reproduces the existing aspect-preserving fit-inside step exactly; actual widths are one pixel below the recorded requested boxes, with no crop or content change.",
