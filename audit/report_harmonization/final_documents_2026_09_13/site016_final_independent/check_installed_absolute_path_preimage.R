...
for (i in seq_len(nrow(protected))) {
  ...
  } else exact(file.path(root, protected$path[[i]]), protected$expected_sha256[[i]], protected$bytes[[i]])
}
stopifnot(sum(protected$classification != "unchanged_live_exact") == 46L)
...
write_json(list(status = "PASS", installed = 914L, candidate = 914L, exact_replacements = 7L,
 corpus_last = TRUE, protected_rows = 4617L, classified_transition_rows = 46L,
...
cat("ORDER016_INDEPENDENT_INSTALLED=PASS live914 candidate914 replacements7 corpus_last protected4617=4571+46 R75+15+15 static53724 HTTP22 browser_pending\n")
