...
qa_name<-if(length(args))args[1]else"final_visual_verification"
stopifnot(grepl("^[A-Za-z0-9_]+$",qa_name))
dest<-file.path(out,qa_name);stopifnot(!file.exists(dest));dir.create(dest)
file.copy(file.path(root,"finalize_visual_evidence.R"),file.path(dest,"verifier_executed.R"))
sha<-function(f){z<-file(f,"rb");on.exit(close(z));paste0(as.character(sha256(z)))}
...
baseline_root<-file.path(project,"audit/manuscript_nature_health/table_layout_revision_2026_09_13")
baseline<-read.csv(file.path(baseline_root,"package_manifest.csv"))
for(i in seq_len(nrow(baseline)))check(paste0("IMMUTABLE_BASELINE_MEMBER_",i),sha(file.path(baseline_root,baseline$path[i]))==baseline$sha256[i])
