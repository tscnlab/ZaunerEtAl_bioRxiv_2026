...
check("darwin_cache_path", length(at) == 1L && any(grepl('return join4(Deno.env.get("HOME") || "", "Library", "Caches", appName);', bundle[at + 0:5], fixed = TRUE)), "macOS normal cache is existing user Library/Caches/quarto")
check("sass_cache_path", any(grepl('join4(path, "sass.kv")', bundle, fixed = TRUE)) && any(grepl('Deno.openKv(kvFile)', bundle, fixed = TRUE)), "SQLite KV resides at normal Sass cache path")
extract <- c(
