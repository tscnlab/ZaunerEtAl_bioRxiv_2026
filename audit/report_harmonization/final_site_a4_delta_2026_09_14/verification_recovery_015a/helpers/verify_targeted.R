...
j <- 'audit/report_harmonization/final_site_a4_delta_2026_09_14'
...
new <- read_html(file.path(j,'candidate_build',route))
...
write.csv(checks,file.path(j,'verification_recovery_015a','evidence',paste0('targeted_',route,'.csv')),row.names=FALSE)
capture.output(sessionInfo(),file=file.path(j,'verification_recovery_015a','evidence',paste0('targeted_',route,'_session.txt')))
