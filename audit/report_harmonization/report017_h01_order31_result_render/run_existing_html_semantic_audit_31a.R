...
evidence_dir <- file.path(
  root,
  "audit/report_harmonization/report017_h01_order31_result_render"
)
...
link_rows <- lapply(seq_along(hrefs), function(i) {
  href <- hrefs[[i]]
  text <- link_text[[i]]
  forbidden <- grepl("\\.qmd($|[?#])", href, ignore.case = TRUE) ||
...
checks <- data.frame(
  ...
  status = c(
    ...
    nrow(link_audit) > 0L && all(
      link_audit$target_exists &
        link_audit$fragment_resolves &
        link_audit$kind != "unsupported_scheme"
    ),
  ...
  evidence = c(
    ...
    sprintf("%d reader links checked", nrow(link_audit)),
...
write.csv(
  checks,
  file.path(evidence_dir, "semantic_status_31a.csv"),
  row.names = FALSE,
  na = ""
)
