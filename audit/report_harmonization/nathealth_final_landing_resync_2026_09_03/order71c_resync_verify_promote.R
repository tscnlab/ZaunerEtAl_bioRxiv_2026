...
# Bounded, no-render implementation for REPORT-018 Order 71c.
# This program performs file, markup, and reference validation only. It never
# sources a QMD, runs Quarto, or calculates a scientific result.
...
dispatch_path <- file.path(
  project_root,
  "audit/report_harmonization/report018_navigation_order71c_dispatch_manifest.csv"
)
require_file(
  dispatch_path,
  "9063745da469a516f19947203f7e8d2e08625cedd6a4fb02d3f29edfccda61fa",
  3014
)

if (mode %in% c("candidate", "promote")) {
  dispatch <- read_exact_manifest(dispatch_path, 18L)
} else {
...
accepted_inventory_path <- file.path(
  project_root,
  "audit/report_harmonization/nathealth_final_landing_integration_2026_09_02/post_promotion_build_inventory.csv"
)
require_file(
...
stopifnot(nrow(accepted_inventory) == 893L, !anyDuplicated(accepted_inventory$path))
...
corpus_pre_sha <- "b807197850b3d9024899da79403f3aec7d716f9be785fe1eefe6500e1ac1e55d"
index_pre_sha <- "c8abe2f9fbfcbe2fc8b4e39e149797a6dc2d74a5735337ed2399fb6e5814eb21"
docx_pre_sha <- "6cd592391f720829ed278a35e81c854ee7fd54eca6a4a4e664faad7b37310f91"
canonical_html_sha <- "438f80545e09eb1844d15c3d4495d7d2d4dbc07bcf039f4c8e4430b18e79ee0c"
canonical_docx_sha <- "74193a7a787ea18d70933b1742e588304a6bf6050c8b43e8a0a0cc352b44a2b8"
...
check_prebuild <- function() {
  ...
  require_file(production_index, index_pre_sha, 29023063)
  require_file(production_docx, docx_pre_sha, 28792333)
  live <- inventory_files(build_root)
  stopifnot(nrow(live) == 893L, symlink_count(build_root) == 0L)
...
build_candidate_index <- function(site_text, manuscript_text) {
  ...
  site_meta_positions <- gregexpr('<meta name="author"', site_text, fixed = TRUE)[[1L]]
  stopifnot(length(site_meta_positions) == 28L, all(site_meta_positions > 0L))
  site_meta_start <- unname(site_meta_positions[[1L]])
  ...
  if (!identical(site_meta, manuscript_meta)) {
    result <- replace_once_fixed(result, site_meta, manuscript_meta, "page metadata")
  } else {
    stopifnot(count_fixed(site_meta, result) == 1L)
  }
  ...
  if (!identical(site_toc, new_toc)) {
    result <- replace_once_fixed(result, site_toc, new_toc, "page TOC")
  } else {
    stopifnot(count_fixed(site_toc, result) == 1L)
  }
  ...
  site_custom_style <- extract_outer_fixed(
    site_text,
    '<style type="text/css">div.manuscript-table,',
    "</style>",
    "accepted-site manuscript style"
  )
  stopifnot(identical(custom_style, site_custom_style))
  stopifnot(count_fixed('id="quarto-embedded-source-code-modal"', result) == 0L)
...
validate_landing <- function(candidate_path, accepted_site_path, write_outputs = TRUE) {
  ...
  table3 <- xml2::xml_find_first(candidate_document, "//*[@id='tbl-near-eye-metrics']")
  table3_groups <- trimws(xml2::xml_text(xml2::xml_find_all(
    table3,
    ".//*[contains(concat(' ',normalize-space(@class),' '),' gt_group_heading ')]"
  )))
  stopifnot(identical(
    table3_groups,
    c("Duration", "Dynamics", "Exposure history", "Level", "Spectrum", "Timing")
  ))
  ...
  s5 <- xml2::xml_find_first(candidate_document, "//*[@id='fig-s5']")
  s5_image <- xml2::xml_find_all(s5, ".//img")
  stopifnot(length(s5_image) == 1L)
  s5_src <- xml2::xml_attr(s5_image, "src")
  stopifnot(startsWith(s5_src, "data:image/svg+xml;base64,"))
  s5_raw <- jsonlite::base64_dec(sub("^[^,]+,", "", s5_src))
  stopifnot(
    length(s5_raw) > 0L,
    grepl("<svg", rawToChar(s5_raw), fixed = TRUE),
    grepl("two-panel", xml2::xml_attr(s5_image, "alt"), ignore.case = TRUE)
  )

  s12 <- xml2::xml_find_first(candidate_document, "//*[@id='fig-s12']")
  s12_image <- xml2::xml_find_all(s12, ".//img")
  stopifnot(length(s12_image) == 1L)
  s12_description <- paste(
    xml2::xml_attr(s12_image, "alt"),
    trimws(xml2::xml_text(xml2::xml_find_first(s12, ".//figcaption")))
  )
  stopifnot(!grepl("MDER", s12_description, ignore.case = TRUE))
  ...
  summary <- data.frame(
    metric = c(
      ...
      "brown_svg_bytes", "brown_svg_sha256_exact", "s5_svg_two_panel",
      "s12_mder_legend_absent", "table3_metric_order", "docx_links", "toc_links",
    ...
    observed = c(
      ...
      TRUE, TRUE, paste(table3_groups, collapse = " | "),
    ...
    expected = c(
      28, 28, 19, 2762, 0, 20, 74, 124, 0, 0, 108600, TRUE, TRUE, TRUE,
      "Duration | Dynamics | Exposure history | Level | Spectrum | Timing", 1, 42, 20,
...
validate_build <- function(root, accepted_site_path, label, write_outputs = TRUE) {
  ...
  require_file(file.path(root, "ZaunerEtAl2026_NatHealth_phase3_brown.docx"), canonical_docx_sha, 28793619)
  ...
  stopifnot(
    setequal(changed, c("index.html", "ZaunerEtAl2026_NatHealth_phase3_brown.docx")),
    length(added) == 0L,
    length(removed) == 0L,
  ...
  for (index in seq_along(routes)) {
    ...
    dom_rows[[index]] <- data.frame(
      ...
      search_count = length(xml2::xml_find_all(document, "//*[@id='quarto-search']")),
      footer_count = length(xml2::xml_find_all(document, "//footer[contains(concat(' ',normalize-space(@class),' '),' footer ')]")),
      page_navigation_count = length(xml2::xml_find_all(document, "//nav[contains(concat(' ',normalize-space(@class),' '),' page-navigation ')]")),
  ...
  stopifnot(
    ...
    all(dom$header_count == 1L),
    all(dom$search_count == 1L),
    all(dom$footer_count == 1L),
    all(dom$page_navigation_count == 1L),
    all(links$resolved)
  )
...
validate_browser_evidence <- function(prefix) {
  ...
  stopifnot(
    ...
    setequal(unique(route_qa$width), c(390L, 708L, 1440L)),
    all(table(route_qa$route) == 3L),
    all(route_qa$pass),
    sum(route_qa$classification == "CLEAN_NEW_LANDING_PAGE") == 3L,
    sum(route_qa$classification == "ACCEPTED_LEGACY_OVERFLOW_BASELINE_PRESERVED") == 1L,
    sum(route_qa$classification == "ACCEPTED_PREEXISTING_LEGACY_OVERFLOW_BASELINE_PRESERVED") == 1L,
    sum(route_qa$classification == "CLEAN_UNCHANGED_ROUTE") == 106L,
    nrow(landing_qa) == 3L,
    setequal(landing_qa$width, c(390L, 708L, 1440L)),
    all(landing_qa$pass),
...
run_candidate <- function() {
  ...
  live <- check_prebuild()
  if (length(candidate_root_members) == 0L) {
    copy_tree_exact(build_root, candidate_build)
  } else {
    stopifnot(identical(candidate_root_members, "candidate_build"), dir.exists(candidate_build))
  }
  retained_candidate <- inventory_files(candidate_build)
  stopifnot(
    nrow(retained_candidate) == 893L,
  ...
  promotion <- data.frame(
    ...
    sha256 = c(transition$candidate_sha256, docx_transition$candidate_sha256),
    bytes = c(transition$candidate_bytes, docx_transition$candidate_bytes),
    action = c("REPLACE_ONCE", "REPLACE_ONCE"),
    stringsAsFactors = FALSE
  )
  checks <- data.frame(
    check = c(
      "dispatch_18_of_18", "accepted_build_893", "candidate_build_893",
      ...
      "table3_metric_order", "s5_two_panel_svg", "brown_svg_exact",
      "s12_without_mder_legend", "docx_exact", "local_references_resolve",
      "no_render_invoked"
    ),
    pass = TRUE,
    detail = c(
      "18/18", "893/893", "893", "0", "891", "36", "0 duplicates and 0 unresolved IDREF/header tokens",
      "28", "19", "20", "2762", "124",
      "Duration | Dynamics | Exposure history | Level | Spectrum | Timing", "SVG", brown_svg_sha,
      "no MDER legend", canonical_docx_sha,
  ...
  write_csv_atomic(docx_transition, file.path(evidence_dir, "candidate_docx_transition.csv"))
  ...
  cat(
    paste0(
      "ORDER71C_CANDIDATE=PASS index_sha256=", transition$candidate_sha256,
...
run_promote <- function() {
  ...
  backup_docx <- file.path(backup_root, "preimages/ZaunerEtAl2026_NatHealth_phase3_brown.docx")
  backup_corpus <- file.path(backup_root, "preimages/phase4_corpus_manifest.csv")
  copy_atomic(production_index, backup_index)
  copy_atomic(production_docx, backup_docx)
  copy_atomic(corpus_path, backup_corpus)
  backup_manifest <- data.frame(
    path = c(backup_index, backup_docx, backup_corpus),
    source_path = c(production_index, production_docx, corpus_path),
    sha256 = c(sha_file(backup_index), sha_file(backup_docx), sha_file(backup_corpus)),
    bytes = c(bytes_file(backup_index), bytes_file(backup_docx), bytes_file(backup_corpus)),
    recoverable = TRUE,
    stringsAsFactors = FALSE
  )
  ...
  if (!promoted) {
    ...
    copy_atomic(backup_docx, production_docx)
  ...
  cat(
    paste0(
      "ORDER71C_PROMOTION=PASS index_sha256=", sha_file(production_index),
      " docx_sha256=", sha_file(production_docx),
...
run_postflight <- function() {
  ...
  backup_docx <- file.path(backup_root, "preimages/ZaunerEtAl2026_NatHealth_phase3_brown.docx")
  backup_corpus <- file.path(backup_root, "preimages/phase4_corpus_manifest.csv")
  require_file(backup_index, index_pre_sha, 29023063)
  require_file(backup_docx, docx_pre_sha, 28792333)
  ...
  checks <- data.frame(
    check = c(
      ...
      "table3_metric_order", "s5_two_panel_svg", "brown_svg_exact",
      "s12_without_mder_legend", "docx_exact", "local_references_resolve",
      "candidate_browser_111", "production_browser_111", "landing_browser_3x2",
    ...
    detail = c(
      ...
      "Duration | Dynamics | Exposure history | Level | Spectrum | Timing", "SVG", brown_svg_sha,
      "no MDER legend", canonical_docx_sha,
      paste0(sum(production$links$resolved), "/", nrow(production$links)), "111/111", "111/111",
  ...
  completion_path <- file.path(evidence_dir, "order71c_completion.md")
  completion <- paste0(
  ...
  completion_manifest_path <- file.path(evidence_dir, "order71c_completion_manifest.csv")
  ...
  cat(
    paste0(
      "ORDER71C_POSTFLIGHT=PASS index_sha256=", sha_file(production_index),
      " docx_sha256=", sha_file(production_docx),
...
if (identical(mode, "candidate")) run_candidate()
if (identical(mode, "promote")) run_promote()
if (identical(mode, "postflight")) run_postflight()
