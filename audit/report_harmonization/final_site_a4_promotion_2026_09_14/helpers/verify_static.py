...
def verify(mode):
    ...
    build=CANDIDATE if mode=='candidate' else ACTIVE_SITE
    checks=[]
    ...
    for route in routes:
        ...
        if route in ['index.html','supplementary_information.html']:
            ...
            old=Counter(doc(BASELINE/route).xpath('//@id'))
    ...
    check('Exact corpus at selected phase',ACTIVE_CORPUS.read_bytes()==(original if mode=='candidate' else prospective))
