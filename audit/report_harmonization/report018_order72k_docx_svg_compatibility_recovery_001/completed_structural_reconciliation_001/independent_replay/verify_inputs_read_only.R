...
audit_output <- "/private/tmp/order72k-docx-package-independent.S6mIE2"
stopifnot(identical(phase, "after_embedding"))
...
target <- file.path(audit_output, paste0(phase, "_preservation_rows.csv"))
...
write.csv(six, file.path(audit_output, paste0(phase, "_six_execution_inputs.csv")), row.names = FALSE)
...
write.csv(aliases, file.path(audit_output, paste0(phase, "_source_version_aliases.csv")), row.names = FALSE)
writeLines(c("Structural and checksum verification only; no scientific computation.", paste("Phase:", phase),
 ...
 capture.output(sessionInfo())), file.path(audit_output, paste0(phase, "_session.txt")))
if (phase == "before_patch") {
