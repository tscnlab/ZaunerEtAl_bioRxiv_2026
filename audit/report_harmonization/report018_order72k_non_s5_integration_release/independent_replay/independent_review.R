...
selection_section <- function(id) {
  lines <- strsplit(selection, "\n", fixed = TRUE)[[1]]
  label <- sub("^fig-s", "", id)
  start <- grep(paste0("^#### Supplementary Figure S", label, "(\\.| and Table )"), lines)
  stopifnot(length(start) == 1L)
  later <- which(seq_along(lines) > start & grepl("^#{1,4} ", lines))
  end <- if (length(later)) min(later) - 1L else length(lines)
  paste(lines[start:end], collapse = "\n")
}
for (id in c("fig-s5", "fig-s7", "fig-s15", "fig-s17")) {
  sel <- selection_section(id)
...
svg_mapping <- jsonlite::fromJSON(resolve("audit/report_harmonization/report018_order72d_writer_svg_integration/combined_accepted_svg_manifest.json"))$accepted_figures
add("twenty_numbered_figures", nrow(svg_mapping) == 20L && sum(svg_mapping$appearances) == 21L, "Explicit frozen mapping has 20 figure sources and 21 appearances; selection source mixes HTML figures and heading/image/caption blocks")
...
svg_live <- pin_rows(svg_mapping$path)
add("all_twenty_current_svg_sources", all(svg_live$sha256 == svg_mapping$sha256), "All 20 existing display SVG sources are byte-exact, including historical held S5")
resource_paths <- c("manuscript/R0_NatHealth/manuscript_displays.css", "manuscript/R0_NatHealth/references_merged.bib", "nature.csl", "assets/reference.docx", "_extensions/kapsner/authors-block/authors-block.lua", list.files("manuscript/R0_NatHealth/display_assets", recursive = TRUE, full.names = TRUE), list.files("manuscript/R0_NatHealth/editable_tables", recursive = TRUE, full.names = TRUE))
resource_paths <- resource_paths[!file.info(resource_paths)$isdir]
table_map <- jsonlite::fromJSON(resolve("manuscript/R0_NatHealth/_word_test/table_pngs_v2/word_table_png_manifest.json"), simplifyVector = FALSE)
table_images <- unlist(lapply(table_map, function(x) vapply(x$files, function(y) y$path, character(1))), use.names = FALSE)
all_pins <- unique(c(all_pins, svg_mapping$path, resource_paths, table_images))
