# Unchanged mathematical estimand functions copied from the pinned historical implementation.
...
make_estimator_data <- function(bundle, nodes) {
  ...
  if (
    ...
    stop("Selected bundle does not use frozen F3/R3/Q2/Q1/D0.", call. = FALSE)
  }
...
derive_sample <- function(sample_id, model_path) {
  ...
  if (isTRUE(bundle$fit_gate$structural_failure)) {
    stop(
      sprintf("Selected model failed its gate: %s", sample_id),
      call. = FALSE
    )
  }
