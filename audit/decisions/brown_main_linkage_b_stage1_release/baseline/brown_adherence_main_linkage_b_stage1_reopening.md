...
Change ID: `CHG-156`  
Date: 2026-09-11  
Status: draft; author linkage choice recorded; owner execution not yet released  
...
Release barrier: the proposed scope below is not dispatch authority until the
BA-017 and CHG-156 ledger entries, non-circular release manifest, and independent
release checks are complete and the coordinator sends the exact final release
identities. No owner work is authorized by the presence of this draft.
...
and Brown owner relayed this direction on 11 September. It chooses the new
primary linkage; it does not accept replacement estimates that do not yet
exist.
...
`stage1_handoff.md` as the return. Include R audit code, aggregate support and
reassignment tables, input pins, an existing-output coverage matrix, a
model/compute plan, a downstream dependency matrix, verification evidence,
session/command records, and a non-circular final manifest in that root.
...
Use R 4.6.1 for every scientific audit. Work from the sealed frame object,
including deterministic filtering of `linkage_b` by finite
`support_fraction >= 0.80`. Revalidate actual-date/UTC ownership and the
recorded Brown numerator/denominator rather than rebuilding scientific minutes.
Hash existing inputs before and after. New audit code or evidence corrections
within this new root may be completed together before the final Stage 1 seal.
...
4. Complete, separately versioned BA-M1 through BA-M6 families for both B
   samples, preserving their definitions, weighting, contrast directions,
   endpoint structure, and multiplicity hierarchy. Recompute the whole
   relevant family, including Daytime and Sleep results, because joint-model
   dependence and FDR adjustment preclude splicing in C estimates.
5. The B-to-B coverage claim gate: directions, interval-exclusion decisions,
   ...
   C-to-B linkage comparison is not this coverage gate.
...
7. Separate B chest construction and comparison, retaining the accepted
   measurement-position qualifications. Do not pool positions or change the
   underlying measurement interpretation through the linkage amendment.
