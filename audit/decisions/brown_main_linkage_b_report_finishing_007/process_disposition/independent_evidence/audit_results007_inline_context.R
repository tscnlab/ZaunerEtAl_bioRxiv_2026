...
addition_path <- file.path(
  scope_dir,
  "approved_source/added_inline_contract.csv"
)
pins <- read.csv(file.path(scope_dir, "dispatch_manifest.csv"))
pin <- pins[pins$path == addition_path, ]
stopifnot(nrow(pin) == 1L, sha(addition_path) == pin$sha256)
added <- read.csv(addition_path, check.names = FALSE)
stopifnot(nrow(added) == 8L)
inventory <- rbind(
  data.frame(
    document = "results",
    expression = sub("`$", "", sub("^`r ", "", added$expression)),
