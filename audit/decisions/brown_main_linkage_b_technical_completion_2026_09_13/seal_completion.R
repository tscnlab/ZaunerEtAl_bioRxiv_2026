# Exact final infrastructure inventory after independent report acceptance.
...
if (args[1] == "verify") {
  ...
  m <- verify(target)
  cat(
    "BROWN_TECHNICAL_COMPLETION=PASS members=",
    nrow(m),
    " manifest=",
    sha(target),
    "\n",
...
stopifnot(
  startsWith(
    package,
    file.path(brown, "main_linkage_b_amendment/stage4/reporting/finishing_006/")
  ),
  sha(package) == args[3]
)
m <- verify(package)
...
executions <- c(
  ...
  "finishing006_preservation_execution_001",
  "finishing006_package_execution_001"
)
...
for (n in c(
  ...
  "audit_methods006_formula_outputs.R",
...
paths <- unique(c(
  ...
  file.path(
    ...
    c(
      ...
      "results_internal_independent_acceptance.md"
    )
  ),
...
m <- verify(target)
cat(
  "BROWN_TECHNICAL_COMPLETION_SEAL=PASS members=",
  nrow(m),
  " manifest=",
  sha(target),
  " acceptance=",
