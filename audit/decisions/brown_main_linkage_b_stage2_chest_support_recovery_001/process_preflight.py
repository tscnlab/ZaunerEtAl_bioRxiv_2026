"""Read-only process infrastructure check. No science or signal operations."""
import datetime
import json
import pathlib
import re
...
matches = [line for line in rows if any(x in line for x in needles) and engine.search(line)
           and "process_preflight.py" not in line and "seal_dispatch.R" not in line]
record = {"utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
          "command": run.args, "exit_code": run.returncode,
          "no_competing_brown_process": not matches, "matching_count": len(matches),
