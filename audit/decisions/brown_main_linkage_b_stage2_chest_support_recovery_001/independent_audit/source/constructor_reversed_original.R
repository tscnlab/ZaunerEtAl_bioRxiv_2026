...
job <- read.csv(file.path(registry_root, "job_registry_0011.csv"))
stopifnot(
  nrow(job) == 1L,
  !anyNA(job),
  !dir.exists(job$output_root),
  sha256(file.path(code_root, "18_construct_chest_b.R")) == job$driver_sha256
)
relative_output <- "placement/chest_b_frames"
...
checks <- data.frame(
  check = c(
    ...
    "nine_registered_sites",
    "all_36_category_cells",
  ...
  pass = c(
    ...
    identical(levels(frame$site), site_levels),
    nrow(support) == 36L,
