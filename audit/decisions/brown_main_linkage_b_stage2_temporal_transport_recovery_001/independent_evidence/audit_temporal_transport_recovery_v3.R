...
audit_partial_path <- file.path(stage2_root, "temporal/inputs/ANY.rds")
partial_hash <- sha256(audit_partial_path)
...
for (s in c("ANY", "80")) {
  ...
  new <- if (s == "ANY") readRDS(audit_partial_path) else
    readRDS(file.path(
      out,
      "temporal/inputs_recovery_001",
      paste0(s, ".rds")
...
audit_check(
  ...
  sha256(audit_partial_path) == partial_hash && sha256(frame_path) == frame_hash
)
original_manifest_assert(old_manifest_path)
audit_check(
