...
qmd_path <- file.path(root, "notebooks/hypotheses/H01.qmd")

stopifnot(file.exists(html_path), file.exists(qmd_path), dir.exists(evidence_dir))
...
extract_source_ids <- function(pattern) {
  hits <- regmatches(qmd_lines, gregexpr(pattern, qmd_lines, perl = TRUE))
  unique(unlist(hits, use.names = FALSE))
}
source_table_ids <- extract_source_ids("tbl-h01-[a-z0-9-]+")
source_figure_ids <- extract_source_ids("fig-h01-[a-z0-9-]+")
...
header_reference_rows <- lapply(table_endpoints, function(endpoint) {
  ...
  if (!length(nodes)) {
    return(data.frame(
      endpoint = character(),
      element = character(),
      header_token = character(),
      matches_in_table = integer(),
      resolves_to_th = logical(),
      pass = logical(),
      stringsAsFactors = FALSE
    ))
  }
  do.call(rbind, lapply(nodes, function(node) {
    tokens <- strsplit(trimws(html_attr(node, "headers")), "[[:space:]]+")[[1]]
    ...
    do.call(rbind, lapply(tokens, function(token) {
      ...
      data.frame(
        ...
        resolves_to_th = length(matches) == 1L && identical(xml_name(matches[[1]]), "th"),
        pass = length(matches) == 1L && identical(xml_name(matches[[1]]), "th"),
        stringsAsFactors = FALSE
      )
    }))
  }))
})
header_reference_audit <- do.call(rbind, header_reference_rows)
...
idref_rows <- lapply(idref_attributes, function(attribute) {
  ...
  do.call(rbind, lapply(nodes, function(node) {
    raw_value <- html_attr(node, attribute)
    tokens <- strsplit(trimws(raw_value), "[[:space:]]+")[[1]]
    tokens <- tokens[nzchar(tokens)]
    do.call(rbind, lapply(tokens, function(token) {
...
checks <- data.frame(
  check = c(
    ...
    "table_endpoint_set_and_order_matches_source",
    "principal_table_is_first",
    "all_tables_semantically_complete",
    "principal_table_present",
    "exact_labelled_figures",
    "figure_endpoint_set_and_order_matches_source",
    "principal_figure_is_first",
    "all_figures_semantically_complete",
    "principal_figure_present",
    "unique_document_ids",
    "all_headers_tokens_resolve_once_within_own_table",
    ...
    "exact_deviation_link_occurrences_and_unique_anchors",
    "all_source_data_links_resolve",
    "all_registered_country_coded_site_names_present",
    "active_h01_navigation"
  ),
  status = c(
    length(table_endpoints) == 36L && length(unique(table_ids)) == 36L,
    identical(table_ids, source_table_ids),
    length(table_ids) > 0L && identical(table_ids[[1]], "tbl-h01-primary-publication-summary"),
    ...
    length(figure_ids) > 0L && identical(figure_ids[[1]], "fig-h01-model-support"),
    nrow(figure_audit) == 10L && all(figure_audit$pass),
    "fig-h01-model-support" %in% figure_ids,
    length(duplicate_ids) == 0L,
    nrow(header_reference_audit) > 0L && all(header_reference_audit$pass),
  ...
  evidence = c(
    ...
    sprintf("%d source labels and %d rendered labels in identical order", length(source_table_ids), length(table_ids)),
    if (length(table_ids)) table_ids[[1]] else "no table endpoint",
    ...
    sprintf("%d source labels and %d rendered labels in identical order", length(source_figure_ids), length(figure_ids)),
    if (length(figure_ids)) figure_ids[[1]] else "no figure endpoint",
    ...
    sprintf("%d header tokens checked; %d failures", nrow(header_reference_audit), sum(!header_reference_audit$pass)),
    sprintf("%d explicit IDREF tokens checked; %d failures", nrow(idref_audit), sum(!idref_audit$pass)),
    ...
    sprintf("%d occurrences; %d unique anchors", nrow(deviation_links), length(unique(deviation_links$fragment))),
    sprintf("%d displayed source-data links", nrow(source_data_links)),
    paste(names(site_label_presence)[site_label_presence], collapse = "; "),
...
write.csv(
  idref_audit,
  file.path(evidence_dir, "document_idref_audit.csv"),
  row.names = FALSE,
  na = ""
)
write.csv(
