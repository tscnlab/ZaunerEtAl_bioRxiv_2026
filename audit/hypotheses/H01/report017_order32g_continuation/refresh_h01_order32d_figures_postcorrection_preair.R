...
durable_check <- bind_rows(lapply(
...
if (
  ...
  stop(
    "Figure 1 sealed-to-source raster transition is not exact",
    call. = FALSE
  )
}
...
geometry_check <- bind_rows(lapply(names(candidate_paths), function(current_figure_id) {
  ...
  if (nrow(current_spec) != 1L) {
    stop("A candidate figure does not have exactly one geometry specification", call. = FALSE)
  }
