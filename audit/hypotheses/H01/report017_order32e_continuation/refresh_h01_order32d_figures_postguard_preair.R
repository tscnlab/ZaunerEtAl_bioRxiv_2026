...
baseline_specs <- tribble(
  ~figure_id, ~width, ~height, ~png_width, ~png_height, ~png_sha256, ~svg_sha256,
  "model_support", 10.5, 7.4, 3360L, 2368L,
  ...
  "paired_placement", 12.0, 6.8, 3840L, 2176L,
  ...
  "diagnostic_assessment", 11.5, 7.6, 3680L, 2432L,
...
if (
  !identical(sort(unique(support_source$support_status)),
             c("Not supported", "Supported")) ||
    !identical(sort(unique(diagnostic_source$check_status)),
               c("Not applicable", "Pass", "Review")) ||
    !identical(sort(unique(paired_source$effect_scale)), c("Difference", "Ratio")) ||
    !all(paired_source$sample_exactly_matched)
) {
  stop("A frozen display category or matched-sample flag is invalid", call. = FALSE)
}
...
make_support_plot <- function(candidate = FALSE) {
  ...
  plot <- support_source |>
    mutate(
      manuscript_name = factor(.data$manuscript_name, levels = metric_levels_support),
      question_label = factor(.data$question_label, levels = question_levels),
      placement_label = factor(.data$placement_label, levels = c("Near eye", "Chest"))
    ) |>
    ...
    geom_text(aes(label = .data$support_symbol), size = symbol_size, colour = "#111111") +
...
build_set <- function(output_dir, candidate = FALSE) {
  list(
    model_support = save_pair(
      make_support_plot(candidate), output_dir, "model_support", 10.5, 7.4
    ),
    paired_placement = save_pair(
      make_paired_plot(candidate), output_dir, "paired_placement", 12.0, 6.8
    ),
    diagnostic_assessment = save_pair(
      make_diagnostic_plot(candidate), output_dir,
      "diagnostic_assessment", 11.5, 7.6
    )
  )
}
...
label_boxes <- function(svg_path, labels) {
  ...
  values <- xml2::xml_text(nodes)
  if (!identical(sort(values), sort(labels))) {
    stop("A paired-placement point label is missing from the SVG", call. = FALSE)
  }
  styles <- xml2::xml_attr(nodes, "style")
  parse_number <- function(pattern, text) {
    as.numeric(sub(paste0(".*", pattern, "([0-9.]+).*"), "\\1", text))
  }
  x <- as.numeric(xml2::xml_attr(nodes, "x"))
  y <- as.numeric(xml2::xml_attr(nodes, "y"))
  width <- as.numeric(xml2::xml_attr(nodes, "textLength"))
  font_size <- vapply(styles, function(value) {
    match <- regmatches(value, regexpr("font-size: [0-9.]+px", value))
    as.numeric(sub("font-size: ", "", sub("px", "", match, fixed = TRUE), fixed = TRUE))
  }, numeric(1))
...
font_sizes <- function(svg_path) {
  ...
  styles <- xml2::xml_attr(nodes, "style")
  values <- xml2::xml_text(nodes)
  sizes <- vapply(styles, function(value) {
    match <- regmatches(value, regexpr("font-size: [0-9.]+px", value))
...
path_rows <- function(paths, set_name) {
  bind_rows(lapply(names(paths), function(figure_id) {
    tibble(
      set = set_name,
      figure_id = figure_id,
      format = names(paths[[figure_id]]),
      path = unname(paths[[figure_id]]),
      sha256 = vapply(unname(paths[[figure_id]]), sha256_file, character(1)),
      bytes = as.numeric(file.info(unname(paths[[figure_id]]))$size)
    )
  }))
}
...
baseline_check <- output_inventory |>
  ...
  left_join(
    baseline_specs |>
      select("figure_id", expected_png = "png_sha256", expected_svg = "svg_sha256"),
    by = "figure_id",
...
if (!all(baseline_check$png_exact) || !all(baseline_check$svg_exact)) {
  stop("Source-derived baseline did not reproduce every sealed figure", call. = FALSE)
}
...
if (!all(geometry_check$width_exact & geometry_check$height_exact & geometry_check$dpi_exact)) {
  stop("A candidate figure has unexpected geometry or DPI", call. = FALSE)
}

structure_check <- bind_rows(lapply(names(candidate_paths), function(figure_id) {
  baseline_svg <- baseline_paths[[figure_id]][["svg"]]
  candidate_svg <- candidate_paths[[figure_id]][["svg"]]
  node_equal <- identical(svg_inventory(baseline_svg), svg_inventory(candidate_svg))
  text_equal <- identical(svg_text_inventory(baseline_svg), svg_text_inventory(candidate_svg))
  tibble(
    figure_id = figure_id,
...
if (!all(structure_check$node_structure_equal & structure_check$text_multiset_equal)) {
...
write_csv(paired_collisions, file.path(attempt_dir, "paired_label_collisions.csv"))
if (nrow(paired_collisions) != 0L) {
  stop("The paired-placement candidate retains a label collision", call. = FALSE)
}
...
if (any(min_typography$minimum_final_size_pt_708 < 7)) {
  stop("A required matrix text element remains below 7 pt at 708 px", call. = FALSE)
}

source_evidence <- source_specs |>
  mutate(
    current_sha256 = vapply(file.path(root, .data$path), sha256_file, character(1)),
...
parameters <- tribble(
  ~figure_id, ~parameter, ~baseline_value, ~candidate_value,
  "model_support", "status_symbol_size_mm", "3.2", "4.0",
  "model_support", "required_text_size_pt", "theme-derived", "11.2",
  "paired_placement", "box_padding_lines", "0.22", "0.55",
  "paired_placement", "point_padding_lines", "0.14", "0.32",
  "paired_placement", "force", "default", "8",
  "paired_placement", "force_pull", "default", "0.05",
  "paired_placement", "max_iter", "default", "100000",
  "paired_placement", "max_time_seconds", "default", "10",
  "diagnostic_assessment", "status_symbol_size_mm", "3.0", "4.3",
  "diagnostic_assessment", "required_text_size_pt", "theme-derived", "12.2"
)
...
attempt_log <- tibble(
  ...
  geometry_pass = all(
    geometry_check$width_exact & geometry_check$height_exact & geometry_check$dpi_exact
  ),
...
cat(
  ...
  "MINIMUM_FINAL_TEXT_PT=", min(min_typography$minimum_final_size_pt_708), "\n",
  "PAIRED_LABEL_COLLISIONS=", nrow(paired_collisions), "\n",
  sep = ""
)
