...
audit_manifest <- function(
  relative,
  expected_rows,
  allowed_changed_paths = character()
) {
  ...
  manifest$classification <- ifelse(
    manifest$status == "PASS",
    "exact",
    ifelse(
      manifest$path %in% allowed_changed_paths,
      "authorized_result_html_change",
      "unclassified_drift"
    )
  )
  stopifnot(!any(manifest$classification == "unclassified_drift"))
  manifest
}
...
authorized_result_html <-
  "_build/nathealth/notebooks/hypotheses/H01.html"
dispatch <- audit_manifest(
  dispatch_relative,
  12L,
  allowed_changed_paths = authorized_result_html
)
source_acceptance <- audit_manifest(
